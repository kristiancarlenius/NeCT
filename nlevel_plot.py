...
def load_config_data(config_dir, ref_epoch_arr, ref_time_arr, crops):
    losses_file = config_dir / "epoch_losses.txt"
...
def collect_all(crops, ref_epoch_arr, ref_time_arr):
    ...
    all_data = {}
    for d in sorted(SIZEDIFF.iterdir(), key=lambda p: int(FOLDER_RE.match(p.name).group(1))
                    if p.is_dir() and FOLDER_RE.match(p.name) else -1):
        if not d.is_dir() or not FOLDER_RE.match(d.name):
            continue
        label = d.name
        print(f"  Loading {label} ...", flush=True)
        all_data[label] = load_config_data(d, ref_epoch_arr, ref_time_arr, crops)
    return all_data
