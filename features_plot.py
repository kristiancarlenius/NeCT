#!/usr/bin/env python3
"""SSIM/PSNR/MAE vs epoch/time plots for n_features_per_level sweep (23_X_23 folders)."""
...
RESULTS = ROOT / "results" / "features_plots"
...
FOLDER_RE = re.compile(r"^23_(\d+)_23$")
...
def collect_all(crops, ref_epoch_arr, ref_time_arr):
    """Returns {label: data_dict} for every 23_X_23 folder, sorted by n_features."""
    dirs = sorted(
...
def plot_epoch(all_data, metric):
    ...
    ax.legend(fontsize=8, title="23_features_23", loc="lower right")
    ax.grid(True, alpha=0.3)
    _finish(fig, RESULTS / f"features_epoch_{metric}.png",
            f"n\_features sweep (n\_levels=23, hashmap=23) — {METRICS[metric]} vs Epoch")
...
def plot_time(all_data, metric):
    ...
    ax.legend(fontsize=8, title="23_features_23", loc="lower right")
    ax.grid(True, alpha=0.3)
    _finish(fig, RESULTS / f"features_time_{metric}.png",
            f"n\_features sweep (n\_levels=23, hashmap=23) — {METRICS[metric]} vs Time")
...
def plot_vram_efficiency(all_data, metric):
    ...
    ax.grid(True, alpha=0.3)
    _finish(fig, RESULTS / f"features_vram_{metric}.png",
...
def main():
    ...
    if not all_data:
        print("No 23_X_23 folders found in", SIZEDIFF)
        return
