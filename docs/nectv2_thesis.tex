...
\begin{figure}[htbp]
    ...
    \caption{High-level overview of the two contributions of this thesis. The NeCT QuadCubes configuration serves as the shared starting point. Part~\ref{part:encoder_optimisation} investigates the encoder design space, reducing peak training VRAM by approximately 60\,\% and improving reconstruction quality by 1.84\,dB PSNR, with the \MixedCubes{} architecture as the best-performing result. Part~\ref{part:continuous_scanning} extends the NeCT Beer--Lambert forward model to account for the angular integration that arises during fly-scan (continuous rotation) acquisition.}
