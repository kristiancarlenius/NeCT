...
The CombinedCubes result demonstrates that the temporal encoders in QuadCubes are over-provisioned relative to what this signal requires. Replacing the three large 3D temporal encoders with three small collision-free 2D encoders does not weaken temporal distinction; reconstruction quality improves on two of three metrics. The 2D encoders are sufficient for the lower-dimensional, lower-frequency temporal signal. CombinedCubes is not a weaker temporal model but a more efficiently allocated one, focusing compute where it is most productive.
...
\begin{figure}[H]
    ...
    \end{adjustwidth}
    \caption{PSNR at approximately 18 hours of training versus peak reserved VRAM for each architecture and configuration. Each point is one experimental run; models are colour-coded by architecture. The near-vertical spread within each architecture shows that the capacity tradeoffs identified in the sweeps translate directly to hardware requirements.}
...
\begin{figure}[H]
    ...
    \end{adjustwidth}
    \caption{PSNR vs.\ epoch for QuadCubes with an enlarged spatial encoder (\texttt{xyz}) relative to the three temporal encoders. The smaller spatial-heavy config ($L=23$, $\log_2 T_\text{spatial}=23$, 20.1\,GB) already reaches 36.65\,dB at epoch~500 versus the uniform baseline's 36.47\,dB at 44.8\,GB. The larger config ($L=24$, $\log_2 T=24$, 24.1\,GB) reaches 37.86\,dB.}
...
The results confirm the asymmetry hypothesis decisively. The smaller spatial-heavy config (20.1\,GB) outperforms the uniform baseline (44.8\,GB) on all three metrics at 24\,h: PSNR 36.60 vs 35.89\,dB ($+0.71$\,dB), SSIM 0.920 vs 0.912, MAE 2.3 vs 2.5 --- while using less than half the VRAM. Enlarging the spatial encoder further (24.1\,GB) raises PSNR to 37.53\,dB ($+1.65$\,dB) at 24\,h, with SSIM 0.934 and MAE 2.1. Temporal-heavy configurations tell the opposite story: the larger (36.5\,GB) reaches only 34.46\,dB ($-1.43$\,dB) at 24\,h, and the smaller (34.4\,GB) only 31.68\,dB ($-4.21$\,dB). Both consume nearly as much VRAM as the baseline yet produce markedly inferior reconstructions. The conclusion is unambiguous: hashmap capacity allocated to the temporal encoders returns far less quality per byte than the same capacity allocated to the spatial encoder.

\todo{Insert PSNR/SSIM/MAE table for all five configurations at 14 and 24 hours.}
...
Figures~\ref{fig:combinedcubes_reconstruction} and~\ref{fig:combinedcubes_quality} show the initial 150-epoch comparison. CombinedCubes achieves higher PSNR and lower MAE than QuadCubes at that checkpoint, with QuadCubes retaining a marginal SSIM advantage. Over the full training run (Figures~\ref{fig:combinedcubes_epoch} and~\ref{fig:combinedcubes_time}), the advantage deepens. At epoch~500 the best CombinedCubes configuration (\texttt{24\_4\_24\_6\_128}, 24.3\,GB) reaches 38.88\,dB PSNR, SSIM 0.945, and MAE 1.8, compared to the QuadCubes baseline's 36.47\,dB, SSIM 0.921, and MAE 2.4. At the 24-hour wall-clock mark, \texttt{18\_4\_24\_4\_128} (18.3\,GB) achieves 37.28\,dB PSNR, SSIM 0.926, and MAE 2.1 --- a gain of $+1.39$\,dB over the baseline (35.89\,dB) at 59\% less VRAM. Even a memory-efficient configuration (\texttt{22\_4\_22\_6\_128}, 8.3\,GB) reaches 36.24\,dB PSNR at 24\,h, approaching baseline quality at one fifth the VRAM. This is consistent with the hypothesis that the QuadCubes temporal encoders are over-provisioned: the freed capacity, redirected to a lighter but collision-free 2D temporal encoder, improves rather than harms reconstruction quality.
...
CombinedCubes takes this finding to its logical structural conclusion. By replacing the three temporal 3D encoders with 2D encoders, it simultaneously reduces memory by 67\%, reduces training time by 30\%, and matches or exceeds the baseline on two of three quality metrics. \MixedCubes{} takes this a step further by replacing the hashed 2D temporal encoders with fully dense grids, guaranteeing an exact coordinate-to-feature correspondence. Early results are promising, though the tradeoff against the less flexible memory footprint of dense grids relative to hashed variants warrants attention.

The third design principle follows from this. The encoder architecture should reflect the asymmetry between the spatial and temporal components of the problem. The spatial encoder benefits from high-dimensional encoding with adequate hashmap size, while the temporal encoders benefit from low-dimensional, collision-free encoding that is cheap to evaluate and suited to the lower-frequency temporal signal.
...
\begin{itemize}
    ...
    \item \textbf{\MixedCubes{} (best).} \texttt{24\_4\_24\_4\_128} from \cref{subsec:mixedcubes}, using a 3D spatial hash encoder and three fully dense 2D temporal encoders at 22.1\,GB VRAM. This is the highest-scoring architecture at the 24-hour mark (37.73\,dB PSNR, SSIM 0.936, MAE 2.0), and the highest-scoring at epoch~500 (38.25\,dB PSNR, SSIM 0.940, MAE 1.9).
\end{itemize}
...
\begin{table}[h]
    ...
    \begin{tabular}{llllll}
        ...
        \MixedCubes{} / CombinedCubes (best)  & \todo{} & \todo{} & \todo{} & \todo{} & 14 \\
        \midrule
        NeCT baseline (QuadCubes)   & \todo{} & \todo{} & \todo{} & \todo{} & 24 \\
        Optimal QuadCubes           & \todo{} & \todo{} & \todo{} & \todo{} & 24 \\
        \MixedCubes{} / CombinedCubes (best)  & \todo{} & \todo{} & \todo{} & \todo{} & 24 \\
        \bottomrule
    \end{tabular}
    \end{adjustwidth}
\end{table}
...
\begin{figure}[H]
    ...
    \end{adjustwidth}
    \caption{PSNR vs.\ epoch for all three QuadCubes configurations. The baseline (\texttt{23\_4\_23\_4\_128}, 44.8\,GB) reaches 36.47\,dB at epoch~500. The mid-size config (\texttt{22\_4\_22\_4\_128}, 23.1\,GB) reaches 36.08\,dB, and the smallest (\texttt{21\_4\_21\_4\_128}, 11.9\,GB) reaches 35.04\,dB.}
...
\begin{figure}[H]
    ...
    \end{adjustwidth}
    \caption{PSNR vs.\ epoch for all architectures. Each model family is shown in a distinct colour. At epoch~500 the top configurations are: CombinedCubes 38.88\,dB (24.3\,GB), \MixedCubes{} 38.25\,dB (22.1\,GB), spatial-heavy QuadCubes 37.86\,dB (24.1\,GB), and the uniform QuadCubes baseline 36.47\,dB (44.8\,GB). Temporal-heavy QuadCubes lag well behind at 34.79 and 31.77\,dB despite comparable VRAM.}
...
CombinedCubes and \MixedCubes{} address a fundamentally different inefficiency, the misallocation of capacity between spatial and temporal encoders. By exploiting the physics of the reconstruction problem, both achieve a much larger reduction in memory and training time than capacity reduction alone, while maintaining or improving reconstruction quality. \MixedCubes{} offers an additional precision guarantee through its collision-free dense temporal grids.
