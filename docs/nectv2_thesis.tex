...
The second objective is to characterise the relationship between encoder capacity and reconstruction quality in the NeCT QuadCubes architecture. The NeCT paper used the largest feasible configuration and trained for as long as possible, treating quality as the only optimisation target. Because NeCT trains from scratch for every new scan, training time is reconstruction time, and there is no pre-trained model to fall back on. Every hour saved in training translates directly into a faster reconstruction turnaround for the experimenter. This thesis asks whether there is a meaningful quality-versus-cost Pareto frontier: configurations that are substantially cheaper in memory and training time while achieving competitive reconstruction quality. Understanding this frontier is a prerequisite for using NeCT on hardware that is more widely accessible than an A100 or H100 GPU, and for workflows where reconstructions must be available quickly enough to guide ongoing experiments.
...
This chapter investigates two interrelated questions about the NeCT encoder that the original paper left unaddressed: whether the QuadCubes structural decomposition of the 4D space is optimal, and whether the capacity assigned to each encoder is appropriate. These questions are explored in sequence, each building on the previous finding. The chapter is organised around three sections corresponding to the three governing phenomena, covered in \cref{sec:collision_and_dim} for collision rate and hashmap size, \cref{sec:levels_and_coupling} for level count and coordinate coupling, and \cref{sec:spatial_temporal_asym} for the spatial-temporal capacity asymmetry. Each section opens with a quantitative characterisation of the phenomenon within QuadCubes and then examines the architecture that makes its consequences most visible.
...
\begin{figure}[H]
    ...
    \centering
    \includegraphics[width=1.3\textwidth]{hashmap23_plots/hashmap23_epoch_psnr.png}
    \end{adjustwidth}
...
\begin{figure}[H]
    \begin{adjustwidth}{-2.5cm}{-2.5cm}
    \centering
    \includegraphics[width=1.3\textwidth]{hashmap23_plots/hashmap23_time_psnr.png}
    \end{adjustwidth}
...
\begin{figure}[H]
    \begin{adjustwidth}{-2.5cm}{-2.5cm}
    \centering
    \includegraphics[width=1.3\textwidth]{hashmap23_plots/hashmap23_vram_psnr.png}
    \end{adjustwidth}
...
\begin{figure}[H]
    \begin{adjustwidth}{-2.5cm}{-2.5cm}
    \centering
    \includegraphics[width=1.3\textwidth]{nlevel_plots/nlevel_epoch_psnr.png}
    \end{adjustwidth}
...
\begin{figure}[H]
    \begin{adjustwidth}{-2.5cm}{-2.5cm}
    \centering
    \includegraphics[width=1.3\textwidth]{nlevel_plots/nlevel_time_psnr.png}
    \end{adjustwidth}
...
\begin{figure}[H]
    \begin{adjustwidth}{-2.5cm}{-2.5cm}
    \centering
    \includegraphics[width=1.3\textwidth]{nlevel_plots/nlevel_vram_psnr.png}
    \end{adjustwidth}
...
\begin{figure}[H]
    \begin{adjustwidth}{-2.5cm}{-2.5cm}
    \centering
    \includegraphics[width=1.3\textwidth]{features_plots/features_epoch_psnr.png}
    \end{adjustwidth}
...
\begin{figure}[H]
    \begin{adjustwidth}{-2.5cm}{-2.5cm}
    \centering
    \includegraphics[width=1.3\textwidth]{features_plots/features_time_psnr.png}
    \end{adjustwidth}
...
\begin{figure}[H]
    \begin{adjustwidth}{-2.5cm}{-2.5cm}
    \centering
    \includegraphics[width=1.3\textwidth]{features_plots/features_vram_psnr.png}
    \end{adjustwidth}
...
The U-Net decoder exploits the multi-resolution structure of the hash grid by processing the $L$ encoder levels at matched spatial scales rather than collapsing all levels into a single flat vector. The levels of each encoder are partitioned into three scale groups. The first $l_\text{coarse}$ levels form the coarse group, the next $l_\text{medium}$ levels the medium group, and the remaining $L - l_\text{coarse} - l_\text{medium}$ levels the fine group. Features from all six encoders are concatenated at each scale, giving three vectors $\mathbf{c}$, $\mathbf{m}$, $\mathbf{f}$ of dimensions $6\,l_\text{coarse}\,F$, $6\,l_\text{medium}\,F$, and $6\,(L - l_\text{coarse} - l_\text{medium})\,F$ respectively.
...
SexCubes demonstrates the limit of this reasoning. Collision-free 2D encoders with low per-step cost achieve ample frequency coverage but fail to encode the higher-order coordinate interactions that the MLP needs to reconstruct 3D spatial structure. Increasing decoder depth or using a U-Net decoder can partially compensate, but both approaches show diminishing returns as the decoder attempts to infer three-way and four-way interactions from pairwise features alone. The second design principle follows from this. The encoder dimensionality must be high enough to capture the coordinate interactions that the reconstruction problem requires, and the number of levels should be tuned to the training time budget rather than maximised.
...
The two preceding sections treated all four QuadCubes encoders uniformly: the same hashmap size, the same number of levels, the same features per level. The theory in \cref{subsec:spatial_temporal_theory} gave reason to expect that this is suboptimal, because the spatial encoder and the temporal encoders have different gradient update densities, different signal properties, and therefore different capacity requirements. This section tests that hypothesis quantitatively through non-uniform sizing experiments within QuadCubes, and then demonstrates its structural consequence in the CombinedCubes and \MixedCubes{} architectures.
...
\todo{Insert representative reconstruction slices for all five configurations at a matched 19-hour budget.}

\begin{figure}[H]
    \begin{adjustwidth}{-2.5cm}{-2.5cm}
    \centering
    \includegraphics[width=1.3\textwidth]{psnr_arc/quadcubes_large_spatial_epoch_psnr.png}
    \end{adjustwidth}
...
\begin{figure}[H]
    \begin{adjustwidth}{-2.5cm}{-2.5cm}
    \centering
    \includegraphics[width=1.3\textwidth]{psnr_arc/quadcubes_large_spatial_time_psnr.png}
    \end{adjustwidth}
    \caption{PSNR vs.\ wall-clock time for spatial-heavy QuadCubes configurations. At 24\,h the smaller spatial-heavy config achieves PSNR 36.60\,dB, SSIM 0.920, MAE 2.3, versus the uniform baseline's 35.89\,dB, SSIM 0.912, MAE 2.5, using less than half the VRAM (20.1\,GB vs 44.8\,GB).}
...
\begin{figure}[H]
    \begin{adjustwidth}{-2.5cm}{-2.5cm}
    \centering
    \includegraphics[width=1.3\textwidth]{psnr_arc/quadcubes_large_temporal_epoch_psnr.png}
    \end{adjustwidth}
...
\begin{figure}[H]
    \begin{adjustwidth}{-2.5cm}{-2.5cm}
    \centering
    \includegraphics[width=1.3\textwidth]{psnr_arc/quadcubes_large_temporal_time_psnr.png}
    \end{adjustwidth}
    \caption{PSNR vs.\ wall-clock time for temporal-heavy QuadCubes configurations. At 24\,h the configs reach 34.46\,dB (SSIM 0.876, MAE 3.0, 36.5\,GB) and 31.68\,dB (SSIM 0.785, MAE 4.3, 34.4\,GB) respectively, both substantially below the 35.89\,dB baseline despite comparable VRAM budgets.}
    \label{fig:temporal_heavy_time}
\end{figure}

The results confirm the asymmetry hypothesis decisively. The smaller spatial-heavy configuration (20.1\,GB) outperforms the uniform baseline (44.8\,GB) on all three metrics at 24\,h, with PSNR 36.60 vs 35.89\,dB ($+0.71$\,dB), SSIM 0.920 vs 0.912, and MAE 2.3 vs 2.5, while using less than half the VRAM. Enlarging the spatial encoder further (24.1\,GB) raises PSNR to 37.53\,dB ($+1.65$\,dB) at 24\,h, with SSIM 0.934 and MAE 2.1. Both spatial-heavy variants reach their gains within the 24-hour budget and converge faster per epoch than the baseline, because the reduced temporal encoder size lowers the per-step memory footprint and allows larger effective batch sizes.

Temporal-heavy configurations tell the opposite story across both the epoch and the time plots. Despite consuming 34--37\,GB, the two temporal-heavy variants plateau well below the uniform baseline and show no sign of closing the gap with additional training. The epoch curves diverge early and maintain a stable offset, ruling out the possibility that the temporal-heavy models simply need more time. The quality deficit is structural: additional hashmap capacity in the temporal encoders cannot represent spatial information the 2D temporal planes do not encode, and the MLP cannot recover spatial precision that the encoders never captured. The conclusion is unambiguous: hashmap capacity allocated to the temporal encoders returns far less quality per byte than the same capacity allocated to the spatial encoder, and increasing temporal capacity beyond the baseline allocation is actively harmful.

\todo{Insert PSNR/SSIM/MAE summary table for all five configurations at 14 and 24 hours.}
...
\begin{figure}[H]
    \begin{adjustwidth}{-2.5cm}{-2.5cm}
    \centering
    \includegraphics[width=1.3\textwidth]{psnr_arc/combinedcube_epoch_psnr.png}
    \end{adjustwidth}
...
\begin{figure}[H]
    \begin{adjustwidth}{-2.5cm}{-2.5cm}
    \centering
    \includegraphics[width=1.3\textwidth]{psnr_arc/combinedcube_time_psnr.png}
    \end{adjustwidth}
...
A consistent observation across CombinedCubes configurations is that deeper decoders (6 hidden layers rather than 4) produce more stable training curves and slightly higher final PSNR. This is not observed within QuadCubes, where decoder depth has negligible effect. The explanation follows from the architectural difference: QuadCubes overlaps all four encoders pairwise, meaning every coordinate interaction is represented redundantly across multiple encoders. CombinedCubes separates the spatial and temporal encoders more cleanly, so the MLP receives less redundant information and benefits from slightly more decoding capacity to reconstruct the final attenuation value. This additional capacity requirement is modest (6 versus 4 hidden layers), confirming that the architectural separation does not fundamentally burden the decoder.
...
\todo{Describe the \MixedCubes{} dense grid configuration: base resolution, scale factor, number of levels, and total memory cost of the three dense temporal encoders. Compare with the hashed CombinedCubes memory footprint.}

\begin{figure}[H]
    \begin{adjustwidth}{-2.5cm}{-2.5cm}
    \centering
    \includegraphics[width=1.3\textwidth]{psnr_arc/mixedcubes_epoch_psnr.png}
    \end{adjustwidth}
...
\begin{figure}[H]
    \begin{adjustwidth}{-2.5cm}{-2.5cm}
    \centering
    \includegraphics[width=1.3\textwidth]{psnr_arc/mixedcubes_time_psnr.png}
    \end{adjustwidth}
...
As with CombinedCubes, deeper decoders (6 hidden layers) provide more stable convergence in \MixedCubes{}, and this effect is more pronounced than in QuadCubes. The structural reason is the same: with the spatial and temporal components separated into distinct encoders that share no feature dimensions, the MLP receives clearly partitioned rather than redundantly overlapping information, and benefits from modest additional capacity to integrate it.
...
The non-uniform sizing experiments confirm the spatial-temporal asymmetry hypothesis decisively. Redirecting hashmap capacity from temporal to spatial encoders improves quality on all three metrics at substantially lower VRAM. Temporal-heavy configurations, by contrast, perform well below the baseline despite comparable memory budgets. Increasing temporal encoder capacity within the QuadCubes architecture returns far less quality per byte than the same increase applied to the spatial encoder.

CombinedCubes takes this finding to its structural conclusion. By replacing the three temporal 3D encoders with collision-free 2D encoders, it simultaneously reduces memory by 67\%, reduces training time by approximately 30\%, and achieves substantially better reconstruction quality. The best CombinedCubes configuration at 24\,h ($+1.39$\,dB, 18.3\,GB) outperforms the baseline at less than half the VRAM. \MixedCubes{} takes this a step further by replacing the hashed 2D temporal encoders with fully dense grids, guaranteeing an exact coordinate-to-feature correspondence and yielding the best 24-hour result across all architectures ($+1.84$\,dB, 22.1\,GB). Both architectures benefit from slightly deeper decoders (6 hidden layers) compared to QuadCubes, reflecting the fact that the cleaner architectural separation between spatial and temporal components places more decoding responsibility on the MLP, a modest and well-motivated cost.
...
The practical question that follows is which specific encoder should be recommended for practical 4D-CT reconstruction, given the full range of configurations and architectures explored. The best configuration found for each design philosophy is placed in a direct head-to-head comparison against the original NeCT baseline.
...
\begin{table}[h]
    ...
    \centering
    \caption{Full comparison of the NeCT baseline, optimal QuadCubes, and best \MixedCubes{} configuration at equalised training budgets. All results on the Bentheimer dataset. \todo{Should use more than one dataset.}}
    \label{tab:full_comparison}
    \begin{tabular}{llllll}
        \toprule
        Configuration & PSNR (dB) & SSIM & MAE & Memory (GB) & Time (h) \\
        \midrule
        NeCT baseline (QuadCubes)   & \todo{} & \todo{} & \todo{} & 44.8 & 14 \\
        Optimal QuadCubes           & \todo{} & \todo{} & \todo{} & \todo{} & 14 \\
        \MixedCubes{} \texttt{24\_4\_24\_4\_128} & \todo{} & \todo{} & \todo{} & 22.1 & 14 \\
        \midrule
...
Figure~\ref{fig:full_reconstruction_comparison} shows representative reconstruction slices from all three configurations at a matched 14-hour budget. \todo{Insert reconstruction comparison figure. Include slices at the first, middle, and last timestep for each configuration.}

\begin{figure}[H]
    \begin{adjustwidth}{-2.5cm}{-2.5cm}
    \centering
    \includegraphics[width=1.3\textwidth]{psnr_arc/quadcubes_epoch_psnr.png}
    \end{adjustwidth}
...
\begin{figure}[H]
    \begin{adjustwidth}{-2.5cm}{-2.5cm}
    \centering
    \includegraphics[width=1.3\textwidth]{psnr_arc/quadcubes_time_psnr.png}
    \end{adjustwidth}
...
\begin{figure}[H]
    \begin{adjustwidth}{-2.5cm}{-2.5cm}
    \centering
    \includegraphics[width=1.3\textwidth]{psnr_arc/combined_time_psnr.png}
    \end{adjustwidth}
    \caption{PSNR vs.\ wall-clock time for all architectures. At 24\,h the ranking is: \MixedCubes{} \texttt{24\_4\_24\_4\_128} 37.73\,dB (22.1\,GB), spatial-heavy QuadCubes \texttt{24\_4\_24\_4\_128} 37.53\,dB (24.1\,GB), CombinedCubes \texttt{18\_4\_24\_4\_128} 37.28\,dB (18.3\,GB), and the QuadCubes baseline 35.89\,dB (44.8\,GB). CombinedCubes and \MixedCubes{} overtake the baseline early in training and maintain the lead throughout.}
...
The three candidates represent three distinct philosophies rather than arbitrary points on a continuous axis. The NeCT baseline maximises capacity within the QuadCubes structural choice, treating all four encoders equally and making no attempt to exploit the asymmetry between spatial and temporal components. It is the upper bound of the original design paradigm, and the results show it is neither the fastest nor the best: the mid-size uniform QuadCubes configuration achieves essentially identical quality at half the VRAM, and all split-encoder architectures exceed its quality at lower cost.

The optimal QuadCubes configuration stays within the uniform-encoder paradigm but addresses its most wasteful aspect, the combination of extremely high level count and large hashmap that makes each epoch expensive without proportionally improving quality within a practical time budget. Its advantage over the baseline is purely computational. By doing more iterations in the same wall-clock time, it achieves a better result without any change to the underlying architecture.

CombinedCubes and \MixedCubes{} address a fundamentally different inefficiency, the misallocation of capacity between spatial and temporal encoders. By exploiting the physics of the reconstruction problem, both achieve a much larger reduction in memory and training time than capacity reduction alone, while exceeding the baseline reconstruction quality. \MixedCubes{} offers an additional precision guarantee through its collision-free dense temporal grids and achieves the highest quality of any architecture tested at the 24-hour mark.
...
The capacity reduction identified for QuadCubes (optimal level count and hashmap size) and the architectural improvement of CombinedCubes and \MixedCubes{} were developed as separate investigations. The results show that both improvements are substantial independently: reducing QuadCubes capacity to the mid-size configuration halves VRAM with negligible quality loss, while switching to CombinedCubes or \MixedCubes{} yields both large memory savings and a quality increase of more than 1.5\,dB at the 24-hour mark. Whether the two improvements are fully additive, that is, whether applying further capacity reduction to the split-encoder architectures beyond what was tested here would yield additional gains, remains an open question and a natural direction for future work.
...
The static dataset consists of four scans of the same Bentheimer sandstone sample used in the encoder experiments, acquired without fluid invasion so that the attenuation field is constant throughout. One scan was acquired in step-and-shoot mode with 1400 projections and serves as the ground truth reference, reconstructed with the standard NeCT static model to produce a high-quality 3D volume. The remaining three datasets are continuous-scan acquisitions at 1400, 360, and 100 projections respectively, spanning a wide range of angular step sizes. The static 3D model from \cref{subsec:nect_static} is used for all reconstructions in this section, and no temporal encoder is required.
