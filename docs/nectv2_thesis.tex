...
The results reveal a clear three-way split. $F = 1$ fails to converge. The encoder lacks sufficient capacity to represent the spatial features at any point during training, and PSNR remains stuck well below the other configurations. $F = 2$ is surprisingly competitive with $F = 4$ in the epoch plot, reaching a very similar asymptotic PSNR, indicating that halving the feature dimension does not meaningfully limit what the model can ultimately represent. In the wall-clock time plot the two curves are practically indistinguishable, because the reduced input dimension also speeds up each training step, allowing $F = 2$ to keep pace with $F = 4$ within the same time budget. Figure~\ref{fig:vram_features} confirms the practical implication: $F = 2$ achieves equivalent quality at roughly half the peak VRAM. This makes $F = 2$ the preferable choice for memory-constrained hardware without sacrificing reconstruction quality.
...
\begin{figure}[H]
    ...
    \end{adjustwidth}
    \caption{PSNR at approximately 18 hours of training versus peak reserved VRAM for each architecture and configuration. Each point is one experimental run; models are colour-coded by architecture. The spread within each architecture shows how capacity tradeoffs identified in the sweeps translate directly to hardware requirements.}
...
The results confirm the asymmetry hypothesis decisively. The smaller spatial-heavy configuration (20.1\,GB) outperforms the uniform baseline (44.8\,GB) on all three metrics at 24\,h, with PSNR 36.60 vs 35.89\,dB ($+0.71$\,dB), SSIM 0.920 vs 0.912, and MAE 2.3 vs 2.5, while using less than half the VRAM. Enlarging the spatial encoder further (24.1\,GB) raises PSNR to 37.53\,dB ($+1.65$\,dB) at 24\,h, with SSIM 0.934 and MAE 2.1. Temporal-heavy configurations tell the opposite story: the larger (36.5\,GB) reaches only 34.46\,dB ($-1.43$\,dB) at 24\,h, and the smaller (34.4\,GB) only 31.68\,dB ($-4.21$\,dB). Both consume nearly as much VRAM as the baseline yet produce markedly inferior reconstructions. The conclusion is unambiguous: hashmap capacity allocated to the temporal encoders returns far less quality per byte than the same capacity allocated to the spatial encoder.
