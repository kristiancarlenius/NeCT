...
\begin{titlepage}
    \centering
    \vspace*{2cm}
    {\LARGE\bfseries \NeCTv{}: Optimizing and extending \NeCT{} \par}
    \vspace{1.5cm}
...
This chapter investigates two interrelated questions about the NeCT encoder that the original paper left unaddressed: whether the QuadCubes structural decomposition of the 4D space is optiaml, and whether the capacity assigned to each encoder is appropriate. These questions are pursued together rather than in sequence, because the capacity parameters and the architectural choices are controlled by the same underlying physical effects. The chapter is organised around collision rate and hashmap size in \cref{sec:collision_and_dim}, frequency coverage and coordinate coupling in \cref{sec:levels_and_coupling}, and spatial-temporal asymmetry in \cref{sec:spatial_temporal_asym}. Each section begins with the quantitative characterisation of the effect within QuadCubes, then presents the architecture that makes that effect most visible.
...
\end{align}
%
At float16 precision with $L = 16$ levels and $F = 4$ features per level, the memory for a single collision-free encoder grows from approximately 128\,MB at $d = 2$, to roughly 64\,GB at $d = 3$, and to approximately 64\,TB at $d = 4$. The $d = 4$ case is entirely infeasible, and the $d = 3$ collision-free encoder used in QuadCubes, which uses 4 of them, would exceed any single GPU. In practice all encoders operate in the collision regime and the architecture question is how to factor the 4D space across multiple lower-dimensional encoders so that each operates with a manageable collision load.
...
\end{equation}
%
where $\Omega$ is the set of voxels in the reconstruction volume. All three metrics are computed on six 2D slices at three points in time: the first, middle, and last timesteps of the scan. PSNR and MSE are sensitive to large errors at high-contrast boundaries, SSIM reflects perceptual sharpness of material boundaries and MAE is less influenced by rare large errors and more reflective of average accuracy across the volume.

\subsection*{Computational Cost}

Computational cost is measured by two quantities: peak GPU memory consumption and total wall-clock training time from initialisation. The primary cost comparison is at an equalised training budgets with corresponding accuracy per epochs. This equalised-time evaluation isolates the practical question of which configuration produces the best reconstruction within a fixed time budget, independently of how many epochs that represents, as larger models use more time per epoch. All experiments are run on a single A100 GPU with 80\,GB VRAM. 
