...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth]{hashmap23_plots/hashmap23_epoch_psnr.png}
    \caption{PSNR vs.\ epoch}
...
\begin{figure}[H]
    ...
    \centering
    \includegraphics[width=1.0\textwidth]{hashmap23_plots/hashmap23_time_psnr.png}
    \end{adjustwidth}
    \caption{PSNR for $\log_2 T \in \{17,\ldots,23\}$ with $L = 23$ and $F = 4$ fixed. Each curve is one hashmap size; the dashed vertical line marks the 18-hour budget.}
    \label{fig:time_hashmap}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth]{hashmap23_plots/hashmap23_vram_psnr.png}
...
Figure~\ref{fig:psnr_vs_params_hashmap} presents an exceptionally clear picture: every reduction in hashmap size produces a corresponding drop in reconstruction quality, and the effect is consistent across both the epoch and time axes. The separation between curves is large and monotone — there is no ambiguity about the ranking. Each step down in $\log_2 T$ also roughly halves the total encoder memory, so the quality loss is directly tied to the doubling of the collision rate at every hash table entry. Figure~\ref{fig:vram_hashmap} makes the memory cost explicit: PSNR at 18 hours scales directly with peak VRAM, with no configuration offering a favourable quality-per-memory trade-off compared to simply using the largest available hashmap. The conclusion is unambiguous: larger hashmaps are always better, and there is no evidence of saturation at $\log_2 T = 23$ within this dataset. Configurations with smaller hashmaps should only be chosen when GPU memory is the hard constraint.
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.55\textwidth}
        \includegraphics[width=\textwidth]{nlevel_plots/nlevel_epoch_psnr.png}
        \caption{PSNR vs.\ epoch}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.55\textwidth}
        \includegraphics[width=\textwidth]{nlevel_plots/nlevel_time_psnr.png}
        \caption{PSNR vs.\ wall-clock time}
    \end{subfigure}
    \end{adjustwidth}
    \caption{PSNR for $L \in \{17,\ldots,23\}$ with $\log_2 T = 23$ and $F = 4$ fixed. Each curve is one level count; the dashed vertical line marks the 24-hour budget.}
    \label{fig:psnr_vs_params_levels}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.65\textwidth]{nlevel_plots/nlevel_vram_psnr.png}
...
Figure~\ref{fig:psnr_vs_params_levels} reveals a strikingly different picture from the hashmap sweep. In the epoch plot, the curves are tightly clustered: all values of $L$ converge to nearly the same asymptotic PSNR, indicating that the dataset complexity is well within the representational capacity of even the smallest level count tested. Removing the finest levels does not meaningfully limit the model's ultimate precision. What does differ is the speed of convergence — configurations with fewer levels reach their asymptote in fewer epochs, since each step is cheaper to compute.

The wall-clock time plot makes the practical implication clear: $L = 18$ stands out as the best-performing configuration across both axes. It converges faster than larger level counts and achieves the highest PSNR within the 24-hour budget, combining the speed advantage of a shallow hierarchy with sufficient frequency coverage for this dataset. Configurations with $L < 18$ converge quickly but plateau slightly lower, suggesting that 18 levels captures the last meaningful frequency band for this data. Configurations with $L > 18$ offer no quality gain and train more slowly. Figure~\ref{fig:vram_levels} reinforces this picture from a memory perspective: PSNR at 24 hours is nearly flat across all level counts despite the varying VRAM footprint, in sharp contrast to the hashmap sweep. The level count therefore acts primarily as a training-time dial rather than a quality or memory dial, confirming the theoretical prediction.
