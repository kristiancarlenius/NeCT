...
\begin{table}[h]
    ...
    \begin{tabular}{lllll}
        \toprule
        Configuration & \texttt{xyz} $\log_2 T$ & Temporal $\log_2 T$ & VRAM (GB) & $L$ \\
        \midrule
        Uniform (baseline) & 23 & 23 & 44.8 & 23 \\
        Spatial-heavy A    & 23 & 21 & 20.1 & 24 \\
        Spatial-heavy B    & 24 & 19 & 24.1 & 23 \\
        Temporal-heavy A   & 21 & 23 & 36.5 & 23 \\
        Temporal-heavy B   & 19 & 23 & 34.4 & 24 \\
        \bottomrule
    \end{tabular}
\end{table}

\todo{Insert representative reconstruction slices for all five configurations at a matched 18-hour budget.}
...
\begin{figure}[H]
    ...
    \end{adjustwidth}
    \caption{PSNR vs.\ wall-clock time for spatial-heavy QuadCubes configurations. At 18\,h the smaller spatial-heavy config achieves PSNR 36.60\,dB, SSIM 0.920, MAE 2.3, versus the uniform baseline's 35.89\,dB, SSIM 0.912, MAE 2.5, using less than half the VRAM (20.1\,GB vs 44.8\,GB).}
...
\begin{figure}[H]
    ...
    \end{adjustwidth}
    \caption{PSNR vs.\ wall-clock time for temporal-heavy QuadCubes configurations. At 18\,h the configs reach 34.46\,dB (SSIM 0.876, MAE 3.0, 36.5\,GB) and 31.68\,dB (SSIM 0.785, MAE 4.3, 34.4\,GB) respectively, both substantially below the 35.89\,dB baseline despite comparable VRAM budgets.}
    \label{fig:temporal_heavy_time}
\end{figure}

The results confirm the asymmetry hypothesis. The smaller spatial-heavy configuration (20.1\,GB) outperforms the uniform baseline (44.8\,GB) on all three metrics at 18\,h, with PSNR 36.60 vs 35.89\,dB ($+0.71$\,dB), SSIM 0.920 vs 0.912, and MAE 2.3 vs 2.5, while using less than half the VRAM. Enlarging the spatial encoder further (24.1\,GB) raises PSNR to 37.53\,dB ($+1.65$\,dB) at 18\,h, with SSIM 0.934 and MAE 2.1. Both spatial-heavy variants reach their gains within the 18-hour budget and converge faster per epoch than the baseline, because the reduced temporal encoder size lowers the per-step memory footprint and allows larger effective batch sizes.
...
\todo{Insert PSNR/SSIM/MAE summary table for all five configurations at 18 hours.}
