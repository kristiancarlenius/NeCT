...
\begin{figure}[H]
    ...
    \caption{The Transformer architecture with encoder and decoder stacks. Each layer applies multi-head self-attention followed by a position-wise feed-forward network, with residual connections and layer normalisation at each sub-layer.}
...
\begin{figure}[H]
    ...
    \caption{PSNR at epoch 250 as a function of log$_2$ hashmap size, with $F=4$ fixed, shown separately for each number of levels $L$. Each panel sweeps log$_2 T \in \{17,\ldots,23\}$ with all other parameters held constant.}
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \end{adjustwidth}
    \caption{PSNR at epoch 250 as a function of number of levels $L$, with $F=4$ fixed, shown separately for each log$_2$ hashmap size. Each panel sweeps $L \in \{17,\ldots,23\}$ with all other parameters held constant.}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.2\textwidth]{equalized_psnr.png}
...
Figure~\ref{fig:nonuniform_quality} shows representative reconstruction slices for non-uniform configurations compared to the baseline QuadCubes reconstruction at a matched 12-hour wall-clock budget.
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \end{adjustwidth}
    \caption{Reconstruction comparison between QuadCubes and SingleCube.}
    \label{fig:singlecube_reconstruction}
\end{figure}


Figure~\ref{fig:singlecube_reconstruction} shows the SingleCube output exhibits severe noise from the extreme hash collision load in the 4D encoder, to the point where even large clusterings of mineral consentration os only vaugly in the correct area. The models inability to correctly learn features and their placement demonstrate the high degree of collisions mean the deconstruction of the data becomes impossible to converge on. In contrast the QuadCubes decomposition into 3D subproblems produces a clean reconstruction.

 The SingleCube experiment demonstrates that the collision rate is a tangable concern which reduction of is likly to lead to better results. The disambiguation mechanism that allows the QuadCubes encoders to handle moderate collision loads depends on cross-encoder context that is unavailable in a single-encoder design.
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.5\textwidth}
        ...
        \caption{SexCubes 128×4}
    \end{subfigure}
    \hspace{0.1cm} 
    \begin{subfigure}[b]{0.5\textwidth}
        \includegraphics[width=\textwidth]{sexcubes_128_7.png}
        \caption{SexCubes 128×7}
    \end{subfigure}
    \hspace{0.1cm} 
    \begin{subfigure}[b]{0.5\textwidth}
        \includegraphics[width=\textwidth]{sexcubes_128_16.png}
        \caption{SexCubes 128×16}
    \end{subfigure}
    \end{adjustwidth}
    \caption{Reconstruction comparison between QuadCubes and three SexCubes configurations (128 neurons, varying hidden layers). Increasing decoder depth improves SexCubes quality but the architecture remains challenged by the lack of higher-order coordinate coupling.}
    \label{fig:sexcubes_reconstruction}
\end{figure}

Figure~\ref{fig:sexcubes_reconstruction} shows very clearly how dependant this sexcubes architecture is on the decoder. Visually being unable to recognise the image as a sample at 4 hidden levels to seeing clear pattern at 7 hidden levels is a clear sign of this.
Yet we see diminishing returns to the 16 hidden levels, while still better still has clear streak artifacts pressent in the reconstruction and overall is unable to reach comparative precision with QuadCubes.


\subsection{Alternative Decoders}
\label{sec:arch_decoders}

The architectures above all use the same overall MLP, altough hidden layers vary, as the decoder. Observing how dependant the sexcubes design is on the decoder and attempting to achive better results, this section investigates whether a more capable decoder can compensate for the limitations of the encoder.

\subsubsection{Transformer Decoder}
\label{subsec:arch_transformer}

The transformer decoder keeps encoders unchanged and replaces the TCNN MLP with a PyTorch transformer.
The six encoder feature vectors are each linearly projected to a common token dimension $d_\text{model}$ via a shared projection layer, yielding a sequence of six tokens $\mathbf{T} \in \mathbb{R}^{6 \times d_\text{model}}$.
Learnable positional embeddings are added to distinguish the six encoder sources.
The token sequence is then processed by $n_\text{layers}$ identical self-attention blocks, each implementing pre-layer-normalisation multi-head self-attention followed by a position-wise feed-forward network with hidden width $6 \cdot d_\text{model}$ and GELU activation.
Attention scores are computed via \texttt{torch.matmul} over $6 \times 6$ matrices rather than through \texttt{nn.MultiheadAttention}, since the standard PyTorch implementation dispatches to flash-attention CUDA kernels that exceed the hardware grid-size limit when the product of batch size and number of heads is large.
After the final block, the four output tokens are mean-pooled to a single vector of dimension $d_\text{model}$, which is passed through a two-layer output head (linear $\to$ ReLU $\to$ linear) to produce the scalar attenuation prediction.

\todo{Add results + the computational overhead}

\subsubsection{U-Net Decoder}
\label{subsec:arch_unet}

The U-Net decoder exploits the multi-resolution structure that is already present in the hash-grid encoder.
Each hash grid produces $L$ levels of features; the levels are partitioned into three scale groups by the parameters $l_\text{coarse}$ and $l_\text{medium}$: the first $l_\text{coarse}$ levels form the coarse group, the next $l_\text{medium}$ levels form the medium group, and the remaining $L - l_\text{coarse} - l_\text{medium}$ levels form the fine group.
For each of the six SexCubes encoders the output is sliced accordingly, and the slices from all six encoders are concatenated at each scale, giving three vectors $\mathbf{c} \in \mathbb{R}^{6 l_\text{coarse} F}$, $\mathbf{m} \in \mathbb{R}^{6 l_\text{medium} F}$, and $\mathbf{f} \in \mathbb{R}^{6 l_\text{fine} F}$, where $F$ is the number of features per level.

These are processed by a down-path and a symmetric up-path with skip connections, mirroring the U-Net architecture~\cite{ronneberger2015unet} for 1D feature vectors:
%
\begin{align}
...
\end{align}
%
where $[\,;\,]$ denotes vector concatenation and the widths of $\mathbf{e}_1$, $\mathbf{e}_2$, $\mathbf{e}_3$ are controlled by the three hidden dimensions $[d_1, d_2, d_3]$ in \texttt{UNetDecoderConfig}.
A final linear layer maps $\mathbf{d}_1$ to the scalar attenuation output.

\todo{Add results + the computational overhead}

\subsection{Dense Encoders}
...
The dense encoder variant is built directly on top of SexCubes. The six 2D hash-grid encoders are replaced by 2D dense grids

The dense 2D grids, use a TCNN \texttt{Grid} encoding with \texttt{type: Dense}.
Rather than mapping coordinates to hash-table entries, a dense grid allocates one feature vector per grid vertex, so there is a one-to-one correspondence between coordinates and stored features.
The grid resolution at level $\ell$ is $R_\text{base} \cdot s^{\ell - 1}$, where $R_\text{base}$ is the base resolution and $s$ is the per-level scale factor.

\todo{Add results, with each of the encoder types + the computational overhead}
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \end{adjustwidth}
    \caption{High-contrast reconstruction slices comparing the QuadCubes reference, uniform \NeCT{} configuration, and the non-uniform MixedCubes allocation at an equalized 12-hour training budget. Differences are most visible at high-contrast material boundaries.}
...
\begin{figure}[H]
    ...
    \caption{During a scan the sample is rotated while the scanner is stationary. The illustration is a topdown view of the difference between the two methods where the coloured areas depict where the lense is open during the rotation of the sample.}
...
To validate the continuous-scan forward model in isolation from dynamic reconstruction, it was first applied to a static dataset. A static reconstruction does not require the temporal encoders of QuadCubes and uses the 3D hash-encoded MLP described in \cref{subsec:nect_static}. The dataset used was the Bentheimer sandstone sample used in previous datasets but without the fluid invasion. It consists of four different scans of the sample. One standard 1400 projections step-and-shoot which reconstruction we use as the ground truth and three continous scans at different step lengths: 100, 360 and 1400 projections.
Since no extra information can be captured in a static scan by doing it continuously the purpose of this experiment is to demonstrate the reduced angular blurring artefacts using our new forwarding method in contrast from standard NeCT. It also establishes the degree of precision increase possible for reducing angular sampling.
For the 1400 projections continous scan the angle is 360/1400=~0.26 degrees which low enough that we expect effectivly no difference from the stop-and-shoot model using standard NeCT reconstruction. At 360 projections we have 1 degree angles for each projection and 100 projections give 3.6 degrees. With accumulation steps 2, 3, 4, 6 tested we can see the loss in precision given approximatley equal angular degrees such as 360 at 2 accumulation steps =0.5 degrees compaired to 100 projections 6 accumulation steps = 0.6 degrees. Or 360 at 4 accumulation steps = 0.25 beeing equal 1400 at 1 accumilation step = 0.257. 

 \todo{make a table for the scans with different projection amounts, accumilation steps and angular degree so its easy to see}
...
The datasets used was made using an hourglass as seen in \ref{fig:hourglass} placed in a micro-ct. Here we generated 6 different datasets each using 800 projections, using 11000, 5500 and 2750 degrees of rotation for 4 and 2 fps.   

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{hourglassandbox.jpg}
    \caption{The hourglass next to the box its placed in in the scanner}
    \label{fig:hourglass}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{The hourglass in box in the micro-ct scanner.}
...
\begin{table}[h]                                                              
  ...
  \textbf{Total Time} & \textbf{Angle Step} & \textbf{Time Step} \\             
  \midrule                                                                      
  1 & 4 & 800 & 2750° & 261 s & 3.44° & 0.44 s \\                               
  2 & 4 & 800 & 5500° & 233 s & 6.88° & 0.29 s \\                               
  3 & 4 & 800 & 11000° & 223 s & 13.75° & 0.28 s \\
  4 & 2 & 800 & 2750° & 417 s & 3.44° & 0.52 s \\                               
  5 & 2 & 800 & 5500° & 417 s & 6.88° & 0.52 s \\
  6 & 2 & 800 & 11000° & 421 s & 13.75° & 0.53 s \\                             
  \bottomrule                                                                   
  \end{tabular}                                                                 
  \caption{CT acquisition protocol parameters.}                                 
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \end{adjustwidth}
    \caption{First scan, 2750 rotations, 4 fps, 150 epochs high contrast imaging.}
