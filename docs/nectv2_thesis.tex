...
Figure~\ref{fig:singlecube_reconstruction} shows representative reconstruction slices from the SingleCube model alongside the QuadCubes reference.
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        ...
        \caption{SingleCube}
    \end{subfigure}
    \caption{Reconstruction comparison between QuadCubes and SingleCube. The SingleCube output exhibits severe noise from the extreme hash collision load in the 4D encoder, while the QuadCubes decomposition into 3D subproblems produces a clean reconstruction.}
...
Figure~\ref{fig:sexcubes_reconstruction} shows representative reconstruction slices from SexCubes variants alongside the QuadCubes reference.
...
\State Compute $K{+}1$ boundary angles $\{\psi_0, \psi_1, \ldots, \psi_K\} \gets \mathrm{linspace}(\phi_s,\, \phi_e,\, K{+}1)$
\State $\hat{y}_j \gets 0$ \quad \textbf{for all} detector pixels $j$
\For{$k = 0, \ldots, K-1$}
    \State $\phi_k \gets (\psi_k + \psi_{k+1})\,/\,2$ \Comment{Mid-point of sub-interval $k$}
    \State Update source/detector geometry to angle $\phi_k$
    \State Cast rays; sample $P$ equidistant points $\{\mathbf{r}_p^{\phi_k}\}$ along each ray $j$ within the volume
    \State Query network: $\muhat_p \gets f_\theta\!\left(\mathbf{r}_p^{\phi_k},\, t\right)$ for $p = 1,\ldots,P$ \Comment{Same $t$ for all rays and sub-angles}
    \State $\hat{y}_j \gets \hat{y}_j + \dfrac{1}{K} \cdot \dfrac{d_j}{d_{\max}} \sum_{p=1}^{P} \muhat_p$ \Comment{$d_j$: physical traversal length of ray $j$}
\EndFor
\State \Return $\hat{y}_j$ \quad \textbf{for all} $j$ \Comment{Predicted line integral; compare directly to log-domain sinogram}
\Statex
\Statex \textbf{Gradient computation (two-pass, memory-efficient):}
\State \textbf{Pass 1} (no gradients): execute lines 3--9 for all $k$, cache projector outputs $\{\mathbf{r}_p^{\phi_k}, d_j\}$; obtain $\hat{y}_j$
\State Treat $\hat{y}_j$ as a leaf variable; compute loss $\mathcal{L}$ and back-propagate to obtain $\partial\mathcal{L}/\partial\hat{y}_j$
\State \textbf{Pass 2} (with gradients): replay each $k$ using cached outputs; back-propagate $\sum_j (\partial\mathcal{L}/\partial\hat{y}_j)\cdot\text{contrib}_{k,j}$ through $f_\theta$
\end{algorithmic}
\end{algorithm}
