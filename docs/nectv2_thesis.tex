...
\begin{table}[h]
    ...
    \begin{tabular}{llllll}
        \toprule
        Configuration & \texttt{xyz} $\log_2 T$ & Temporal $\log_2 T$ & VRAM (GB) & $L$ & PSNR at 24\,h (dB) \\
        \midrule
        Uniform (baseline) & 23 & 23 & 44.8 & 23 & 35.89 \\
        Spatial-heavy A    & 23 & 21 & 20.1 & 24 & 36.60 \\
        Spatial-heavy B    & 24 & 19 & 24.1 & 23 & 36.96 \\
        Temporal-heavy A   & 21 & 23 & 36.5 & 23 & 34.46 \\
        Temporal-heavy B   & 19 & 23 & 34.4 & 24 & 31.68 \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{figure}[htbp]
    ...
    \end{adjustwidth}
    \caption{PSNR vs.\ wall-clock time for spatial-heavy QuadCubes configurations. At 18\,h the larger spatial-heavy config (24.1\,GB) achieves 36.96\,dB and the smaller (20.1\,GB) achieves 36.60\,dB, both exceeding the uniform baseline (44.8\,GB) while using less than half the VRAM.}
...
The eight \MixedCubes{} configurations shown here were selected from a larger experimental set to best illustrate the range of performance across different memory, training time, and quality trade-offs. Many additional configurations were evaluated; these eight best represent the key dimensions of the design space.
...
\begin{figure}[htbp]
    ...
    \end{adjustwidth}
    \caption{PSNR vs.\ wall-clock time for all architectures at the 24\,h comparison point. \MixedCubes{} \texttt{24\_4\_24\_4\_128} leads at 37.73\,dB (22.1\,GB), followed by spatial-heavy QuadCubes at 37.53\,dB (24.1\,GB), CombinedCubes \texttt{18\_4\_24\_4\_128} at 37.28\,dB (18.3\,GB), and the QuadCubes baseline at 35.89\,dB (44.8\,GB). All split-encoder models overtake the baseline early in training and maintain the lead throughout.}
...
\begin{figure}[htbp]
    ...
    \end{adjustwidth}
    \caption{PSNR at the wall-clock comparison point versus peak reserved VRAM for each architecture and configuration shown. Each point is one experimental run from the representative sets selected for each architecture (8 configurations each for CombinedCubes and \MixedCubes{}, with additional QuadCubes and spatial/temporal-heavy variants). Models are colour-coded by architecture family. The Pareto frontier is occupied by \MixedCubes{} and CombinedCubes configurations, which achieve the highest quality at every memory level. The $F=2$ configurations (triangular markers within the split-encoder families) fall consistently below the $F=4$ frontier despite large hashmaps. Temporal-heavy QuadCubes configurations are dominated by every other approach at any VRAM budget.}
