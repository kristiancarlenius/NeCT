...
Chapter~\ref{chap:background} develops the theoretical background needed to understand the methods. It opens with X-ray CT from first principles, covering Beer--Lambert's law, cone-beam geometry, classical reconstruction algorithms, and the challenges of 4D-CT and sparse-view acquisition. The second half of the background chapter covers the machine learning tools brought to bear on these problems, beginning with the fundamentals of neural networks and the multilayer perceptron, followed by convolutional approaches to CT reconstruction and their limitations, and then the theory of implicit neural representations including Neural Radiance Fields, positional encodings, and the multiresolution hash encoding. The chapter concludes with a detailed description of the NeCT algorithm itself, including both the static 3D model and the dynamic QuadCubes model, the training pipeline, and the hybrid golden-section sampling scheme.

Chapters~\ref{chap:encoder_size} and~\ref{chap:encoder_arch} together form Part~\ref{part:encoder_optimisation}, which addresses the first and second research objectives through a two-stage investigation of the NeCT encoder. Chapter~\ref{chap:encoder_size} derives the memory cost of the baseline NeCT configuration, identifies the GPU hardware constraints that follow from it, and presents a systematic sweep of encoder size parameters including the hashmap capacity, number of levels, and features per level, measuring reconstruction quality and training time jointly across all configurations. The effect of allocating encoder capacity non-uniformly across the four encoders is also examined. Chapter~\ref{chap:encoder_arch} then takes the findings from that sweep as motivation to explore the design space of encoder architectures more broadly, varying the number, dimensionality, and type of encoders. Alternative decoder architectures based on transformers and U-Nets are also evaluated, as is the use of dense (collision-free) encoders for the temporal dimensions.
...
The NeCT QuadCubes architecture encodes a four-dimensional attenuation field using a set of multiresolution hash grids. In the original paper, the encoder configuration was fixed at the largest practically feasible size and held constant throughout all experiments. This part of the thesis relaxes that constraint and asks two related but distinct questions. First, given the QuadCubes architecture as it stands, how sensitive is reconstruction quality to the capacity of its encoders, and can a smaller configuration achieve competitive results faster and with less memory? Second, is the QuadCubes factorisation of the 4D space the right structural choice to begin with, or do alternative encoder designs offer a better balance between representational power, collision rate, and computational cost?

These two questions are addressed in sequence. Chapter~\ref{chap:encoder_size} characterises the quality-versus-cost landscape within the QuadCubes architecture by systematically varying the encoder capacity parameters. The findings from that sweep, in particular the observation that the baseline configuration is substantially over-provisioned for many practical use cases and that the temporal encoders may benefit from a different treatment than the spatial encoder, directly motivate the architectural experiments in Chapter~\ref{chap:encoder_arch}. Together the two chapters provide a comprehensive account of the NeCT encoder design space, from the question of how large a given architecture should be to the question of which architecture to use in the first place.
...
Computational cost is characterised by two quantities: peak GPU memory consumption, measured as the maximum memory allocated during training, and total training time from initialisation. \todo{Might be better to measure convergence time but full convergence takes over 60 hours for standard quadcubes which is much longer than what is practically used and also requires even compute to test.} All experiments are run separately, each using one A100 with 80\,GB VRAM.
...
Figure~\ref{fig:psnr_vs_params_hashmap} shows PSNR as a function of log$_2$ hashmap size for each fixed $L$ value, revealing how hashmap capacity trades off against reconstruction quality at every level count.
...
The overall trend across panels is as expected, with higher hashmap sizes generally yielding higher PSNR. However, several panels show non-monotonic behaviour that complicates a straightforward reading. At $L = 22$, PSNR decreases from $\log_2 T = 22$ to $\log_2 T = 23$, at $L = 19$ the highest PSNR is reached at $\log_2 T = 20$ rather than at the largest table size, and several configurations show lower PSNR at $\log_2 T = 22$ than at $\log_2 T = 21$. These inversions are consistent with run-to-run variance from stochastic training on a single dataset, and averaging over multiple runs or datasets would be needed to determine whether any of them reflect a genuine effect.
\todo{cant say anything conclusive, need avg of multiple runs or datasets or both to reduce randomness in results. missing 21\_4\_23 and 19\_4\_23}
...
Figure~\ref{fig:psnr_vs_params_levels} shows PSNR as a function of $L$ for each fixed log$_2$ hashmap size, isolating the effect of level count on reconstruction quality.
...
To understand whether total parameter count is a reliable predictor of reconstruction quality regardless of which hyperparameter it comes from, all three sweeps above are plotted together against total encoding parameter count in Figure~\ref{fig:psnr_vs_params_joint}. \todo{Insert joint figure.}
...
Figure~\ref{fig:quality_vs_time} shows PSNR for all configurations evaluated in \cref{sec:enc_size_params} at an equalized time budget of approximately 14 hours, with curves grouped by number of levels and the x-axis showing total encoder parameter count.
...
\begin{figure}[H]
    ...
    \end{adjustwidth}
    \caption{PSNR at an equalized training time of approximately 14 hours for all $(L, \log_2 T)$ configurations with $F=4$. Each curve groups configurations with the same number of levels $L$; x-axis shows total encoder parameter count across all four QuadCubes encoders.}
    \label{fig:quality_vs_time}
\end{figure}

When training time rather than epoch count is held fixed, smaller configurations gain a substantial advantage because they complete more training iterations within the same time budget. The highest PSNR at 14 hours is achieved by the $(L=18,\, \log_2 T=21)$ configuration, which has completed approximately 350 epochs by the time the baseline $(L=23,\, \log_2 T=23)$ has finished only 150. A large number of reduced-size configurations outperform the baseline at this time budget, spanning a wide range of level counts and hashmap sizes. The margin of improvement is not marginal but substantial across many of these configurations, indicating that the baseline is far from time-optimal at a 14-hour budget.

There also appears to be a capacity threshold below which the encoder becomes too small to fully represent the dataset, causing these configurations to plateau at a lower PSNR regardless of additional epoch amount. The results establish clearly that time-optimal reconstructions are achievable with encoders much smaller than the baseline. What cannot yet be determined from single runs on one dataset is which specific combination of $L$ and $\log_2 T$ is best in general, since the variance between individual runs is large enough to shuffle the ranking of configurations that perform similarly.

Figure~\ref{fig:quality_vs_vram} shows the same configurations plotted against peak GPU memory consumption. \todo{Insert figure. horizontal lines at the memory limits: 24 GB for RTX 3090, 40 GB for A100 40GB, 80 GB for A100/H100 80GB.}


\subsection*{Training Dynamics}

Beyond the final converged quality, the rate at which a configuration converges matters for practical use. Figure~\ref{fig:training_curves} shows PSNR as a function of training time for a representative selection of configurations. \todo{Section only stays if i run enough models to actual convergence.}
...
Figure~\ref{fig:metric_comparison} shows PSNR, SSIM, and MAE for all configurations, sorted by total parameter count. \todo{Do last, when everything else has been gathered.}
...
In spatial-heavy configurations, a larger fraction of the total parameter budget is allocated to the \texttt{xyz} encoder, with the temporal encoders correspondingly reduced. Table~\ref{tab:nonuniform_configs} lists the specific configurations evaluated.
...
Figure~\ref{fig:nonuniform_quality} shows representative reconstruction slices for non-uniform configurations compared to the baseline QuadCubes reconstruction at a matched 19-hour budget.\todo{Insert after training.}
...
The size sweep in Chapter~\ref{chap:encoder_size} established two key findings that motivate this chapter. First, the baseline QuadCubes configuration is substantially over-provisioned: configurations with far fewer parameters achieve equal or better reconstruction quality within a fixed time budget, meaning the baseline wastes both memory and computation. Second, the non-uniform sizing experiments suggested that the spatial and temporal components of the encoder may have different capacity requirements, with the temporal encoders appearing to be over-provisioned relative to the spatial encoder. Taken together, these findings raise a more fundamental question: rather than simply shrinking the existing QuadCubes encoders, would a different structural decomposition of the four-dimensional input space be more efficient?

This chapter explores that question by treating the number, dimensionality, and type of encoders as free variables. The central tension in designing a hash encoding for a 4D problem is between hash collision rate and dimensional coupling. A single high-dimensional encoder captures all interactions between coordinates directly but suffers from a severe collision problem at any non-theoretical memory budget. Many low-dimensional encoders reduce collisions but capture only low-order dependencies between coordinates, placing greater demands on the decoder to integrate the independent streams of information.
...
The QuadCubes architecture was described in full in \cref{subsec:nect_quadcubes}. It uses four 3D multiresolution hash encoders covering all four possible 3-element subsets of $\{x, y, z, t\}$. Every pair of coordinates appears together in at least two encoders, and the spatial encoder \texttt{xyz} overlaps with each of the three temporal encoders in two dimensions. With the baseline configuration from Chapter~\ref{chap:encoder_size} ($L = 23$, $\log_2 T = 23$, $F = 4$), the four encoders occupy approximately 5.2\,GB of GPU memory at float16. QuadCubes serves as the primary baseline throughout this chapter with its base parameter settings.
...
The experiments so far reveal a consistent pattern. The SingleCube architecture suffers from extreme collisions, QuadCubes handles them at moderate cost, and SexCubes avoids them but places an excessive burden on the decoder. Informed by the observation from Chapter~\ref{chap:encoder_size} that non-uniform capacity allocation between spatial and temporal encoders can be beneficial, the CombinedCubes architecture attempts a more targeted factorisation. It uses one 3D encoder covering the purely spatial dimensions \texttt{xyz} and three 2D encoders covering the space-time pairs \texttt{xt}, \texttt{yt}, and \texttt{zt}:
%
\begin{equation}
    \muhat(\mathbf{r}, t) = \mathrm{MLP}_\Phi\!\Bigl(
        \bigl[
...
\end{equation}
%
The motivation is to decouple the representation of static spatial structure from temporal dynamics in a way that matches the physical structure of the problem. The spatial encoder alone handles the time-invariant elements in a given dataset, while the three 2D temporal encoders each capture how one spatial coordinate co-varies with time.
...
Figure~\ref{fig:combinedcubes_reconstruction} shows a representative reconstruction slice from CombinedCubes alongside QuadCubes at the same epoch count. The slice is taken from the upper right corner of the Bentheimer dataset at the final timestep, after the brine front has reached the top of the sample, providing a direct test of how well CombinedCubes handles the temporal component of the reconstruction. No hyperparameters were tuned for the CombinedCubes architecture beyond replacing the three temporal 3D hash encoders with 2D encoders at $\log_2 T = 20$. All other settings, including learning rate, batch size, MLP dimensions, and training schedule, remain identical to the QuadCubes baseline. Despite this, CombinedCubes reached 150 epochs in approximately 10 hours compared to 14 hours for QuadCubes, a 30\% reduction in time due to the smaller temporal encoders.
...
The CombinedCubes architecture already uses 2D hash encoders for the temporal dimensions that are collision-free at $\log_2 T = 20$. Replacing these with fully dense encoders eliminates the hash function entirely, removing even the theoretical possibility of collisions and potentially improving gradient flow. Since the dense 2D grids store features in a regular array rather than through a hash function, each coordinate maps deterministically to a unique memory location, which avoids any gradient interference between spatially distant points that happen to share a hash bucket.
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \end{adjustwidth}
    \caption{High-contrast reconstruction slices comparing the Ground truth, the uniform \NeCT{} configuration, and CombinedCubes with dense temporal encoders, all at an equalised 12-hour training budget. Differences are most visible at high-contrast material boundaries.}
    \label{fig:nonuniform_quality}
\end{figure}
...
\todo{Present results comparing CombinedCubes with hashed versus dense encoders. evaluate on multiple datasets.}
...
\hspace{1em} Table~\ref{tab:arch_summary} summarises all architectures across reconstruction quality and memory at set training times. \todo{Fill table, at 19, 24, 36 hours.}

\begin{table}[h]
    \begin{adjustwidth}{-2cm}{0cm}
    \centering
    \caption{Summary comparison of all encoder architectures on Bentheimer \todo{should use more than 1}.}
    ...
    \begin{tabular}{llllll}
        ...
        QuadCubes (baseline)    & \todo{} & \todo{} & \todo{} & \todo{} & \todo{} \\
        QuadCubes (optimal finding)    & \todo{} & \todo{} & \todo{} & \todo{} & \todo{} \\
        SexCubes (biggest mlp)     & \todo{} & \todo{} & \todo{} & \todo{} & \todo{} \\
        SexCubes (u-net)     & \todo{} & \todo{} & \todo{} & \todo{} & \todo{} \\
        SexCubes (transformer)     & \todo{} & \todo{} & \todo{} & \todo{} & \todo{} \\
...
Figure~\ref{fig:arch_pareto} plots all dynamic architectures on a quality-versus-memory plane and a quality-versus-time plane. \todo{Insert figures. Mark the QuadCubes baseline.}
...
The encoder optimisation work in Part~\ref{part:encoder_optimisation} concerned the representational structure of the network given a fixed acquisition protocol. This chapter turns to the acquisition side of the problem and asks what happens when the standard step-and-shoot protocol is replaced by continuous rotation scanning, also called fly-scan, in which the sample rotates at a constant angular velocity and the shutter remains open for the full duration of the scan. This change requires a modification to the NeCT forward model and opens up new possibilities for observing fast dynamic processes that would otherwise be missed between frames.
...
To validate the continuous-scan forward model in isolation from the additional complexity of dynamic reconstruction, it was first applied to a static dataset. A static reconstruction does not require the temporal encoders of QuadCubes and uses the 3D hash-encoded MLP described in \cref{subsec:nect_static}.

The dataset consists of four scans of the same Bentheimer sandstone sample used in previous chapters, acquired without fluid invasion so that the sample remains static throughout. One scan uses the standard step-and-shoot protocol with 1400 projections and serves as the ground truth reference. The remaining three are continuous scans at different projection counts, producing different angular step sizes per projection.
...
Since no additional information can be captured in a static scan by imaging it continuously, the purpose of this experiment is to demonstrate that the multi-step forward model reduces angular blurring artefacts compared to standard NeCT reconstruction at $K = 1$. It also establishes the degree of precision recovery possible for different angular step sizes. For the 1400-projection scan the angular step is approximately $0.26\degree$, which is small enough that effectively no difference from step-and-shoot is expected even at $K = 1$. At 360 projections the step is $1\degree$, and at 100 projections it is $3.6\degree$, where angular blurring becomes visible.

Table~\ref{tab:static_cont_configs} reveals useful equivalences across configurations. For example, 360 projections at $K = 4$ yields an effective sub-step of $0.25\degree$, nearly matching 1400 projections at $K = 1$ ($0.26\degree$). Similarly, 100 projections at $K = 6$ gives $0.60\degree$ per sub-step, comparable to 360 projections at $K = 2$ ($0.50\degree$). Comparing reconstruction quality across these matched pairs isolates how well the accumulation approximation compensates for larger angular steps.
...
The static experiment established that the forward model correctly compensates for angular blurring. The dynamic experiment extends this to time-resolved reconstruction, where continuous scanning provides a physical advantage over step-and-shoot. Because the detector captures information continuously, processes that occur within a single exposure interval are reflected in the recorded intensity rather than being missed between frames.
...
The bold entries in Table~\ref{tab:dynamic_cont_configs} highlight matched sub-step values across datasets. For example, dataset 2 ($\Delta\phi = 6.88\degree$) at $K = 2$ has the same effective sub-step of $3.44\degree$ as dataset 1 ($\Delta\phi = 3.44\degree$) at $K = 1$, and dataset 3 ($\Delta\phi = 13.75\degree$) at $K = 4$ also reaches $3.44\degree$. Comparing reconstruction quality across these matched triplets tests whether the accumulation approximation fully compensates for the larger raw angular step, or whether residual information loss from the coarser sampling persists.
...
\todo{For each of the six datasets, present PSNR/SSIM as a function of $K$. Describe improvement from increasing $K$ depends on $\Delta\phi$.}

\todo{Show reconstruction slices at matched $K$ values and for normal reconstruction, for different $\Delta\phi$ to illustrate the visual effect of the accumulation correction.}
