...
Figures~\ref{fig:combinedcubes_reconstruction} and~\ref{fig:combinedcubes_quality} show the results with no hyperparameter tuning beyond the architectural change. CombinedCubes achieves higher PSNR and lower MAE than QuadCubes, with QuadCubes retaining a marginal SSIM advantage. The improvement on two of three metrics while using one third of the encoder memory is consistent with the hypothesis that the QuadCubes temporal encoders are over-provisioned. The capacity freed from the temporal encoders does not hurt reconstruction, and the faster training (approximately 10 hours to 150 epochs versus 14 hours) means CombinedCubes can complete more iterations in the same wall-clock budget.
...
\todo{Present quantitative results for \MixedCubes{} on PSNR, SSIM, and MAE versus CombinedCubes and the NeCT baseline.}
...
\begin{itemize}
    ...
    \item \textbf{\MixedCubes{} (best).} The best-performing configuration from \cref{sec:spatial_temporal_asym}, either CombinedCubes with hashed 2D temporal encoders or \MixedCubes{} with fully dense 2D temporal encoders, as determined by the quantitative comparison in \cref{subsec:mixedcubes}. \todo{Fill in specific settings once results are finalised.}
\end{itemize}
