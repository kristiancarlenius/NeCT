...
The static results show that the correction benefit scales clearly with angular step size. For the 100-projection configuration ($\Delta\phi = 3.60\degree$) the uncorrected $K = 1$ reconstruction receives a combined score of 0.165, indicating severe angular blur. Increasing $K$ improves quality monotonically, reaching 0.500 at $K = 6$. For the 360-projection configuration ($\Delta\phi = 1.00\degree$) the uncorrected score is already 0.492 and rises to 0.647 at $K = 6$. The fact that 100 projections at $K = 6$ (combined score 0.500, effective sub-step $0.60\degree$) nearly matches 360 projections at $K = 1$ (0.492, $1.00\degree$) is consistent with the Shannon--Nyquist theorem applied to angular sampling. Reconstruction quality is governed by the effective sub-step size $\Delta\phi / K$, and the $K$-step correction makes a coarser acquisition largely interchangeable with a finer native acquisition at the same effective angular resolution.

The approximation error of the midpoint rule scales as $\mathcal{O}((\Delta\phi/K)^2)$, meaning it shrinks by a factor of four each time $K$ is doubled. The monotonic improvement with $K$ and the absence of any quality decline at large $K$ confirm that the correction does not introduce new artefacts. The practical implication is that a larger $K$ is warranted for coarser acquisitions while a smaller $K$ is sufficient for finer ones.
...
The frame-rate comparison between datasets with the same $\Delta\phi$ but different acquisition speeds tests a separate question, namely whether higher temporal resolution yields visibly improved tracking of the sand front, independent of the angular blur correction.
