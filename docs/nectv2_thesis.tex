...
This thesis addresses two limitations of the original NeCT QuadCubes configuration. The first part investigates the encoder design space, identifying three governing design principles through systematic parameter sweeps and architectural experiments. Hashmap size is the dominant driver of both model size and reconstruction quality. The number of resolution levels controls training time rather than quality, and reducing it is the primary lever for faster reconstructions. The original QuadCubes baseline over-provisions the temporal encoders relative to the spatial encoder, a structural inefficiency that can be corrected at the architectural level. The proposed \MixedCubes{} architecture, which replaces the temporal hash encoders with collision-free dense grids and the spatial encoder remains a high-capacity 3D hash grid, achieves higher reconstruction quality than the QuadCubes baseline while using half the GPU memory and training in less time. A lightweight variant matches baseline quality at approximately 11\,\% of the original memory requirement, making \NeCTv{} accessible for all CUDA-compatible consumer GPUs for the first time.
...
Neural Computed Tomography (NeCT), introduced by \citet{friis2025nect}, is the first algorithm to extend this INR-based approach to fully four-dimensional CT. Throughout this thesis, \emph{NeCT} refers to the pipeline, training procedure, and forward model introduced in that paper. The specific model architecture used in the original paper, which encodes the 4D attenuation field using four 3D multiresolution hash encoders, is referred to as \emph{QuadCubes}. \emph{\NeCTv{}} refers to the extended and optimised implementation developed in this thesis, which includes new encoder architectures (CombinedCubes, \MixedCubes{}, HexCubes, SingleCube) and the continuous-scan forward model. Rather than reconstructing each time step independently, NeCT represents the entire dynamic experiment as a single continuous function $\mu(\mathbf{r}, t)$ using a hash-encoded neural network, trained jointly on all projections collected over the full duration of the scan. This holistic treatment allows the reconstruction to exploit angular information from all time steps simultaneously, enabling temporal resolutions approaching the time between individual projections with standard laboratory instruments.
...
Because NeCT trains from scratch on every new scan, the cost of a single reconstruction is the full training cost, repeated every time a new dataset is analysed. The configuration adopted in the original paper requires approximately 45\,GB of GPU memory and approximately 40 hours of training on a single A100 GPU \citep{friis2025nect}. A researcher who needs reconstructions from ten independent imbibition experiments must therefore acquire 400 hours of high-end GPU time, with no option to amortise that cost across datasets. This restricts NeCT to facilities with access to the most expensive research-grade hardware and makes it impractical for any workflow that requires rapid feedback, such as adjusting experimental conditions mid-scan based on a preliminary reconstruction. Reducing the training time and memory footprint of NeCT is therefore not a secondary engineering concern but the primary barrier to its routine adoption in laboratory settings.

This thesis builds directly on NeCT with three objectives, corresponding to the three questions the original paper left open. First, whether the encoder architecture is the optimal structural choice and whether alternative designs can improve reconstruction precision or reduce model size. Second, whether the encoder configuration can be reduced in size without a proportional loss of reconstruction quality, identifying the smallest and fastest configuration that remains competitive. Third, whether the NeCT forward model can be extended to handle continuously acquired data in which the sample rotates without stopping between exposures, enabling information-denser acquisitions.
...
Part~\ref{part:background} develops the theoretical foundations. Part~\ref{part:encoder_optimisation} addresses the first and second research objectives through an investigation of the NeCT encoder that treats architectural structure and capacity as interrelated rather than independent questions. Chapter~\ref{chap:encoder_design} is the core of this investigation. It establishes a theoretical framework based on three governing phenomena (collision rate, frequency coverage, and spatial-temporal asymmetry) and then quantifies each through a systematic parameter sweep within QuadCubes before demonstrating its extreme form through a corresponding alternative architecture. The SingleCube, HexCubes, and CombinedCubes architectures emerge naturally from this structure. Chapter~\ref{chap:encoder_comparison} brings all findings together in a unified comparison of the best configurations against the QuadCubes baseline.
...
\begin{figure}[htbp]
    \hspace*{-2.5cm}\includegraphics[width=1.4\textwidth]{thesis_overview.pdf}
    \caption{Overview of the two contributions of this thesis. NeCT QuadCubes serves as the shared baseline. Part~I (Encoder Optimisation) reduces peak VRAM by 60\% and improves reconstruction quality by 1.84\,dB through architecture search, culminating in MixedCubes. Part~II (Continuous Scanning) extends the NeCT forward model to handle angular-integration blur arising from fly-scan acquisition.}
...
\begin{figure}[htbp]
    ...
    \caption{The NeRF pipeline \citep{mildenhall2021nerf}. A continuous 5D function mapping spatial position and viewing direction to colour and density is represented by an MLP. Rays are marched through the volume, and the rendered pixel colour is compared to the observed image to supervise training via backpropagation.}
...
\begin{table}[h]
    \centering
    \caption{Structural analogy between NeRF \citep{mildenhall2021nerf} and attenuation-contrast CT.}
...
A hash table is a data structure that maps keys to values in $\mathcal{O}(1)$ time using a hash function. Given a key $k$, the hash function $h(k)$ computes a table index, and the corresponding value is retrieved directly. When two keys map to the same index, a collision occurs. Unlike classical hash tables, which resolve collisions through chaining or probing, the multiresolution hash encoding of \citet{mueller2022instant} handles collisions implicitly during training, as described below.

The multiresolution component extends this idea across $L$ independent spatial scales. At each level $\ell$ a regular grid covers the input domain at a different resolution, so that coarse levels capture low-frequency global structure and fine levels capture high-frequency local detail. Using multiple scales simultaneously allows the encoding to represent signals across a wide frequency range without requiring an extremely fine single-resolution grid.

Combining hash tables and multiresolution grids gives the multiresolution hash encoding. The encoding maintains $L$ independent levels, each at a different spatial resolution. At coarse levels, where the grid has fewer than $T$ vertices, each vertex maps to a unique table entry (a 1:1 dense mapping). At fine levels, where the number of grid vertices exceeds $T$, the entries are indexed by a spatial hash function $h : \mathbb{Z}^d \to \mathbb{Z}_T$. The total number of trainable encoding parameters is bounded by $T \cdot L \cdot F$, where $F$ is the feature dimension per entry. The resolutions grow as a geometric progression between a coarsest resolution $N_\mathrm{min}$ and a finest resolution $N_\mathrm{max}$:
%
\begin{equation}
...
\begin{figure}[htbp]
    \hspace*{-2cm}\includegraphics[width=1.3\textwidth]{fig_golden_section.pdf}
    \caption{Hybrid golden-section angle sampling across three revolutions. Each panel shows a top-down view of one full $360\degree$ revolution; dots mark projection angles. Revolution~1 places 10 equidistant projections at $0\degree, 36\degree, \ldots, 324\degree$. Each subsequent revolution starts at an additional golden-angle offset ($\varphi_0 \approx 137.5\degree$), so new projections land between existing ones. The combined 30-projection set (rightmost panel) achieves near-uniform angular coverage. The timing strip (top) contrasts step-and-shoot acquisition, which wastes time on motor settling between exposures, with continuous-scan acquisition, which devotes the full revolution time to exposure.}
...
The investigation is structured around three governing phenomena.

First, we vary the hash collision rate, which is controlled primarily by the hashmap size and increases with encoder dimensionality. The collision rate sets the ceiling on reconstruction quality per model parameter and is the dominant driver of model size.
...
\begin{figure}[htbp]
    \hspace*{-2cm}\includegraphics[width=1.3\textwidth]{fig_phenomena_A.pdf}
    \caption{\textbf{Hash collision rate.} Multiple feature-grid vertices mapping to the same hash-table cell corrupt each other's gradients. Low dimensionality ($d=2$) with a large table ($T=2^{20}$) yields near collision-free encoding; raising the input dimension to $d=3$ with the same $T$ produces $2^{10}$ collisions per cell on average.}
    \label{fig:phenomena_collision}
\end{figure}

\begin{figure}[htbp]
    \hspace*{-2.5cm}\includegraphics[width=1.3\textwidth]{fig_phenomena_B.pdf}
    \caption{\textbf{Frequency coverage.} Each resolution level captures a distinct spatial frequency band. Increasing the number of levels from $L=4$ to $L=8$ extends coverage to finer detail (visible in the reconstruction thumbnails) at the cost of slower training.}
    \label{fig:phenomena_freq}
\end{figure}

\begin{figure}[htbp]
    \hspace*{-2cm}\includegraphics[width=1.3\textwidth]{fig_phenomena_C.pdf}
    \caption{\textbf{Spatial-temporal asymmetry.} The QuadCubes baseline allocates equal capacity to all four 3-D sub-encoders (\texttt{xyz}, \texttt{xyt}, \texttt{xzt}, \texttt{yzt}). CombinedCubes instead devotes a large, high-resolution hash grid to the static spatial encoder \texttt{xyz} (which must resolve sharp pore boundaries) and smaller grids to the three temporal encoders (which need only track the slowly evolving fluid front).}
    \label{fig:phenomena_asymmetry}
\end{figure}

Chapter~\ref{chap:encoder_design} works through these phenomena in sequence. For each, the chapter first characterises it quantitatively through a systematic parameter sweep within the NeCT QuadCubes architecture, then examines an alternative architecture that either pushes the phenomenon to a catastrophic extreme, exposing the cost of getting it wrong, or resolves it in a principled way. Hashmap size and collision rate are paired with SingleCube. Level count and coordinate coupling are paired with the HexCubes family. The spatial-temporal capacity asymmetry is paired with CombinedCubes. Chapter~\ref{chap:encoder_comparison} places all findings into a head-to-head comparison against the QuadCubes baseline.
...
The \textbf{hashmap size} $T = 2^{\log_2 T}$ is the number of feature vectors stored per level in the hash table. Because each of the $L$ levels stores at most $T$ vectors of dimension $F$, the total parameter count of the encoder is upper bounded by $T \cdot L \cdot F$. Since each of the $L$ levels stores at most $T$ vectors of dimension $F$, the total grows proportionally to $T$, doubling when $T$ doubles. In practice parameter count is often below this bound at coarse levels, where grids are small enough that all vertices receive dedicated entries. At practical grid resolutions the finest levels have far more grid vertices than $T$ entries, so multiple vertices are mapped to the same stored vector, causing a collision. The collision rate at the finest level is approximately $N_\mathrm{max}^d / T$, where $d$ is the encoder input dimension. A higher $T$ reduces this ratio, which reduces the number of distinct spatial locations sharing each feature vector and therefore increases the representational precision available at fine scales. Hashmap size is thus the primary driver of both model size and reconstruction quality, as it controls the ceiling of spatial precision the encoder can achieve, and each doubling of $T$ roughly doubles both memory and quality headroom.
...
\todo{Add a small illustrative figure here showing a concrete example of a hash encoder with specific values of $T$, $L$, $F$, and $N_\mathrm{max}$, making the relationship between parameters and the resulting grid structure visually clear.}

\subsection{Collision Rate and Encoder Dimensionality}
\label{subsec:collision_dim_theory}

The collision rate depends not only on the hashmap size but critically on the input dimension of the encoder. The term collision-free is used here to mean that the hashmap is large enough to give every grid vertex at the finest resolution level its own unique table entry, so that hash collisions are not forced by capacity. In practice the probability of a random collision from the hash function itself is negligible for good hash functions, so $T \geq N_\mathrm{max}^d$ effectively means zero forced collisions. Here $d$ is the encoder input dimension, $N_\mathrm{max}$ is the finest grid resolution per axis, and $T_\mathrm{free} = N_\mathrm{max}^d$ is the collision-free hashmap size. For a representative finest resolution of $N_\mathrm{max} = 2^{10} = 1024$, the collision-free requirement becomes: For a representative finest resolution of $N_\mathrm{max} = 2^{10} = 1024$ voxels per side:
For $d = 2$ (a 2D encoder operating over coordinate pairs such as $xt$ or $yz$), the collision-free requirement gives $\log_2 T_\mathrm{free} = 20$, corresponding to $T_\mathrm{free} \approx 10^6$ entries. At float16 precision with $L = 16$ levels and $F = 4$ features per level, a single collision-free 2D encoder requires approximately 128\,MB. This is readily achievable on current hardware and is the basis for the 2D temporal encoders in CombinedCubes and \MixedCubes{}.

For $d = 3$ (a 3D encoder as used in QuadCubes), the collision-free threshold rises to $\log_2 T_\mathrm{free} = 30$, or $T_\mathrm{free} \approx 10^9$ entries. A single collision-free 3D encoder would require approximately 64\,GB of memory. This already saturates an 80\,GB A100, leaving no room for other encoders, optimiser states, or activations. The four QuadCubes encoders therefore operate at $\log_2 T = 23$, well below the collision-free threshold, and tolerate a moderate collision load of approximately 128 vertices per table entry at the finest level.

For $d = 4$ (a single encoder operating directly over the full space-time coordinate $(x, y, z, t)$, as in SingleCube), the collision-free threshold reaches $\log_2 T_\mathrm{free} = 40$, or $T_\mathrm{free} \approx 10^{12}$ entries. A single collision-free 4D encoder would require approximately 64\,TB, which is entirely infeasible on any current hardware. In practice all 4D encoders operate at an extreme collision load. This is the central motivation for factoring the 4D space into multiple lower-dimensional encoders, as QuadCubes does, so that each operates in a tractable collision regime.
...
We introduce asymmetry into the NeCT architecture in this work. The asymmetry that we introduce gives different capacity to the spatial and temporal encoders, reflecting the different information that NeCT must represent in each component.
...
The key asymmetry for capacity allocation is not the number of projections received but what signal each encoder must represent. The $xyz$-encoder must capture the full high-frequency spatial structure of the pore geometry, which has sharp material boundaries and requires fine-scale precision. The temporal encoders ($xyt$-, $xzt$-, $yzt$-encoders) must capture a lower-frequency temporal signal, as the fluid front advances slowly and smoothly relative to the spatial detail in our experimental setting, and they do so along only two spatial dimensions at a time rather than three.

Given the asymmetry described above, our hypothesis is that the $xyz$-encoder requires higher capacity, in terms of the number of parameters, than the temporal encoders. The experimental results with NeCT CombinedCubes, presented in \cref{subsec:combinedcubes}, support this hypothesis directly.
...
The hashmap sweep operated entirely within the 3D QuadCubes encoder regime, where the baseline collision rate at $\log_2 T = 23$ is approximately 128 vertices per entry at the finest level, which is moderate and manageable. SingleCube extends the same encoder dimension to 4D with the same hashmap size, immediately increasing the finest-level collision rate to $N_\mathrm{max}^4 / T = 2^{17} \approx 131\,000$ vertices per entry. This is more than a thousand times the collision load of the baseline 3D encoders and occurs at every level that operates in the hashed regime, making reconstruction with the standard hashmap size structurally infeasible.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.70\textwidth]{arc_diagram/singlecube_arc_diagram.png}
    \caption{The SingleCube architecture. A single 4D multiresolution hash encoder takes the full space-time coordinate $(x, y, z, t)$ as input and passes the concatenated multi-level features to a shared MLP decoder.}
...
The number of resolution levels $L$ is the parameter with the largest effect on per-epoch training time and the smallest effect on model size. As established in \cref{subsec:enc_params_theory}, each additional level adds a constant forward-pass cost by requiring an extra set of hash lookups and interpolations per sample point, while the memory contribution of coarse levels is small. This section examines how $L$ trades training time for frequency coverage, with the prediction from \cref{subsec:enc_params_theory} that it should affect convergence speed far more than asymptotic quality. The HexCubes architecture then illustrates what happens when frequency coverage is ample and collision rates are low, but the encoder can represent only pairwise coordinate interactions, a situation that places an unusually heavy burden on the decoder.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.80\textwidth]{arc_diagram/hexcubes_arc_diagram.png}
    \caption{The HexCubes architecture. Six 2D multiresolution hash encoders cover all six possible coordinate pairs from the four space-time dimensions. The concatenated features are decoded by a shared MLP.}
...
CombinedCubes uses one 3D spatial encoder for \texttt{xyz} and three 2D temporal encoders for the space-time pairs \texttt{xt}, \texttt{yt}, and \texttt{zt}. Figure~\ref{fig:combinedcubes_architecture} shows the architecture.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.80\textwidth]{arc_diagram/combinedcubes_arc_diagram.png}
    \caption{The CombinedCubes architecture. One 3D hash encoder covers the spatial coordinates \texttt{xyz} and three 2D hash encoders cover the space-time pairs \texttt{xt}, \texttt{yt}, and \texttt{zt}. Features from all four encoders are concatenated and decoded by a shared MLP.}
    \label{fig:combinedcubes_architecture}
\end{figure}

The encoder decomposition is expressed as:
%
\begin{equation}
    \muhat(\mathbf{r}, t) = \mathrm{MLP}_\Phi\!\Bigl(
        \bigl[
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.80\textwidth]{arc_diagram/mixedcubes_arc_diagram.png}
    \caption{The \MixedCubes{} architecture. One 3D multiresolution hash encoder covers the spatial coordinates \texttt{xyz} and three fully dense 2D grids cover the space-time pairs \texttt{xt}, \texttt{yt}, and \texttt{zt}. The dense temporal grids are collision-free by construction. Features from all four encoders are concatenated and decoded by a shared MLP.}
...
\begin{figure}[htbp]
    ...
    \end{subfigure}
    \end{adjustwidth}
    \caption{High-contrast reconstruction slices comparing ground truth, the QuadCubes baseline, and \MixedCubes{} at an equalised 12-hour training budget.}
...
The practical question that follows is which configuration should be recommended for practical 4D-CT reconstruction. Four configurations spanning the key design choices explored in Chapter~\ref{chap:encoder_design} are compared against each other and against the QuadCubes baseline on a common footing.
...
\begin{itemize}
    \item \textbf{QuadCubes baseline.} The original NeCT QuadCubes configuration from \citet{friis2025nect} with uniform encoders at $L = 23$, $\log_2 T = 23$, $F = 4$. This is the reference point, representing the largest configuration the original paper considered.
...
\begin{table}[h]
    ...
    \centering
    \caption{Comparison of the QuadCubes baseline against spatial-heavy QuadCubes, CombinedCubes, and \MixedCubes{} configurations at 24\,h wall-clock time and at epoch~500. All results on the Bentheimer sandstone dataset. \todo{Should use more than one dataset.}}
    \label{tab:full_comparison}
    \begin{tabular}{llllll}
        \toprule
        Configuration & PSNR (dB) & SSIM & MAE & VRAM (GB) & Time (h) \\
        \midrule
        QuadCubes baseline                          & 35.89 & 0.912 & 2.5 & 44.8 & 24 \\
        ...
        \midrule
        QuadCubes baseline                          & 36.47 & 0.921 & 2.4 & 44.8 & ep.\,500 \\
...
\begin{figure}[htbp]
    ...
    \end{adjustwidth}
    \caption{PSNR, SSIM, and MAE for the four candidate configurations at the 24-hour mark, shown as grouped bars. The consistent ranking across all three metrics confirms that the quality advantage of the split-encoder architectures over the QuadCubes baseline is not specific to any single measure.}
    \label{fig:top2_comparison}
\end{figure}

Figure~\ref{fig:top2_comparison} confirms that the ranking observed in PSNR holds across all three evaluation metrics. SSIM and MAE confirm the same ranking. \MixedCubes{} and CombinedCubes both improve on the baseline in every metric simultaneously, while the spatial-heavy QuadCubes also outperforms the uniform baseline on all three. The results are therefore not an artefact of PSNR sensitivity to particular types of error.
...
In step-and-shoot mode the sample is brought to a complete stop at each angular position before a projection is recorded. Every photon contributing to the detected image therefore passed through the sample at the same angular orientation, so each detector pixel corresponds to a single well-defined ray through the sample at a known fixed angle. The cost of this precision is time. Between each exposure the sample must decelerate, the controller waits for residual vibration to damp out, the shutter opens, the exposure runs, the shutter closes, and the motor accelerates to the next position. For instruments where the individual exposure time is short relative to the motor settling overhead, a substantial fraction of the total scan time is spent moving the sample between angular positions without collecting photons.

In continuous rotation mode, sometimes called fly-scan, the sample rotates at a constant angular velocity throughout the acquisition and the detector records a continuous sequence of exposures, with consecutive frames separated only by the detector readout time rather than motor settling time. The motor overhead between projections is eliminated, so the detector is integrating photons for a larger fraction of the total scan time. If the angular velocity is $\omega$ and the exposure time per frame is $\tau$, each projection is acquired while the sample rotates through an angular interval of
%
\begin{equation}
    \Delta\phi = \omega\,\tau.
...
The choice of $K$ for each dataset is guided empirically by the matched-pair design of the experiments (see Tables~\ref{tab:static_cont_configs} and~\ref{tab:dynamic_cont_configs}), in which two configurations with the same effective sub-step angle are compared directly. If the approximation is adequate and angular integration is the dominant source of error, a matched pair should produce equivalent reconstruction quality regardless of which combination of $\Delta\phi$ and $K$ was used to reach that sub-step. In practice, larger $\Delta\phi$ introduces additional sources of degradation beyond forward model mismatch, such as reduced angular diversity per projection, so exact equivalence is not guaranteed. The matched-pair comparison therefore tests whether the $K$-step correction closes the quality gap sufficiently to make coarser acquisitions practically interchangeable with finer ones, which is a more meaningful criterion for the application than a purely theoretical convergence bound.
...
The static dataset uses the same Bentheimer sandstone sample as the encoder experiments, scanned without fluid invasion so that the attenuation field is constant throughout. Three independent scans were acquired of the same physical sample, each covering one full rotation of $360\degree$. The first is a step-and-shoot acquisition with 1400 projections and serves as the ground truth reference, reconstructed with the standard NeCT static model to produce a high-quality 3D volume. The remaining two are continuous-scan acquisitions at 360 and 100 projections respectively, spanning a wide range of angular step sizes. The standard NeCT static model is used for all reconstructions, with a single 3D \texttt{xyz} hash encoder and no temporal encoder.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=1.0\textwidth]{continous_fail/mae_4fps.png}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=1.0\textwidth]{continous_fail/mae_8fps.png}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=1.0\textwidth]{continous_fail/subangle_vs_mae_total.png}
...
The static results confirm that the $K$-step correction is implemented correctly and produces measurable improvement when angular blur is the limiting factor. For the 100-projection configuration ($\Delta\phi = 3.60\degree$) the combined score rises from 0.242 at $K = 1$ to 0.400 at $K = 6$, with positive gains at every accumulation step. The correction is working. The gains follow a diminishing pattern: the step from $K = 1$ to $K = 2$ contributes 0.120 points while the step from $K = 4$ to $K = 6$ contributes only 0.028 points, suggesting convergence toward a practical quality ceiling for this projection count.

For the 360-projection configuration ($\Delta\phi = 1.00\degree$) the improvement is small throughout, rising from 0.593 at $K = 1$ to 0.708 at $K = 6$. At an angular step of $1\degree$, the forward model mismatch at $K = 1$ is already small enough that additional sub-steps return diminishing benefit. A $1\degree$ sweep is close to the threshold below which angular blur is no longer the dominant source of reconstruction error, and the correction has little to recover.

The 100-projection configuration does not reach 360-projection quality at any value of $K$ tested. The score at $K = 6$ is 0.400, well below the 360-projection score at $K = 1$ of 0.593. This gap reflects the projection count difference, not the angular step size. The $K$-step forward model corrects angular smear within each projection interval but cannot substitute for the additional independent views that additional projections provide. The quality ceiling of a sparse acquisition is set by how many projections were collected, not by how finely each one's angular interval is subdivided.

The approximation error of the midpoint rule scales as $\mathcal{O}((\Delta\phi/K)^2)$, meaning it shrinks by a factor of four each time $K$ is doubled. The monotonic improvement with $K$ and the absence of any quality decline at large $K$ confirm that the correction does not introduce new artefacts.

\subsection{Dynamic Experiment and Sample Geometry}
\label{subsec:cont_disc_dynamic}

The hourglass dynamic experiment produced a negative result: no meaningful correlation was found between accumulation steps $K$, angular step size $\Delta\phi$, frame rate, and reconstruction quality as measured by the volume MAE metric. This outcome is informative. The most probable explanation lies in the geometric properties of the hourglass sample.

Angular blur degrades reconstruction quality when the sample has fine directional structure that is visible from some angles but not others. The blur integrates over a range of angles, effectively averaging projection views that carry different structural information. For a sample such as the Bentheimer sandstone, which contains sharp pore boundaries whose projection varies strongly with angle, this averaging causes real information loss. The hourglass, by contrast, is largely angularly isotropic from the scanner's perspective. Sand is a dense granular medium with near-circular cross-section, and the overall geometry of the two chambers is cylindrically symmetric around the rotation axis. A projection of the hourglass at any angle carries essentially the same structural information as a projection at a nearby angle. Averaging over a wide angular interval therefore does not meaningfully degrade the information content of each projection, and the $K$-step correction has nothing to recover. The model reconstructs the sand distribution accurately regardless of $K$ because the projections are informative regardless of angular smear.

This interpretation suggests that the practical benefit of the continuous-scan forward model is sample-dependent. Samples with strong directional features, such as fine stratification, crack networks, or elongated pores aligned with the rotation plane, are the primary candidates for meaningful improvement. Isotropic or angularly symmetric samples are unlikely to benefit from the correction, and using the hourglass as a test case for the forward model was ill-suited to revealing the benefit that the static sandstone experiment successfully demonstrated. A more appropriate dynamic test would use a sample with directional structure that evolves over time, such as a rock undergoing anisotropic fracture propagation or a layered sample experiencing differential dissolution.

The frame-rate comparison across matched $\Delta\phi$ datasets also shows no significant effect on the volume MAE. This is consistent with the geometric explanation: if the forward model correction does not affect quality for this sample, then neither does the temporal sampling density, because both are attempting to resolve information that the sample geometry makes uniformly accessible.
...
The hourglass results also highlight a fundamental limitation of the volume MAE metric for evaluating the angular blur correction: the metric is insensitive to the correction when the sample is angularly isotropic. A metric that directly measures projection residuals, or that evaluates reconstruction quality on a sample with known directional structure, would be more appropriate for validating the forward model in a dynamic setting. The static sandstone experiment, which uses sharpness and MAE metrics against a high-quality reference scan, is a better-controlled test of the forward model itself.

The approach is applicable beyond the sandstone and hourglass experiments used here. Any dynamic CT application in which the sample undergoes continuous change and the scanner operates in fly-scan mode is a potential candidate, including in-situ mechanical testing and certain industrial inspection workflows. The degree of benefit from the forward model correction will depend on the angular step size per projection, which varies widely across instrument types and acquisition protocols.
...
This thesis set out to address two practical barriers to the wider adoption of NeCT. The first is the high computational cost of each reconstruction, which restricts the method to the most expensive research-grade hardware. The second is the reliance on a step-and-shoot acquisition protocol that leaves a significant fraction of available scan time unused. The two parts of the thesis address these barriers through a systematic investigation of the encoder design space and through a new forward model for continuous rotation scanning.

\section{Summary of Contributions}
\label{sec:conclusion_summary}

\textbf{Encoder capacity sweeps.} Systematic sweeps of the three hash encoder capacity parameters in the QuadCubes architecture identified three governing design principles. Hashmap size is the dominant driver of both model size and reconstruction quality, with a clear quality threshold around $\log_2 T = 21$ below which reconstruction degrades sharply. The number of resolution levels primarily controls training speed rather than asymptotic quality, and reducing from the baseline $L = 23$ to $L = 18$ saves approximately one quarter of per-epoch training time with no quality penalty. Features per level can be halved from $F = 4$ to $F = 2$ with no measurable quality loss. The QuadCubes baseline configuration is far from optimal along any axis of cost.

\textbf{Spatial-temporal asymmetry.} Non-uniform capacity experiments confirmed a structural inefficiency in the QuadCubes baseline. The three temporal encoders receive the same hashmap capacity as the spatial encoder despite encoding a lower-frequency signal that does not require it. Redistributing capacity toward the spatial encoder while reducing temporal encoder size yields both better quality and lower memory consumption at any fixed training budget.

\textbf{CombinedCubes architecture.} Replacing the three temporal 3D hash encoders with collision-free 2D hash encoders reduces encoder memory by approximately 67\% and training time by approximately 30\% relative to the baseline, while improving reconstruction quality. The architectural change exploits the spatial-temporal asymmetry directly.

\textbf{\MixedCubes{} architecture.} Replacing the hashed 2D temporal encoders with fully dense 2D grids eliminates hash collisions in the temporal encoders and achieves the best reconstruction quality of any architecture tested. The best \MixedCubes{} configuration achieves 37.73\,dB PSNR at 24 hours of training at 22.1\,GB GPU memory, compared to the NeCT baseline's 35.89\,dB at 44.8\,GB, a gain of 1.84\,dB at half the memory. A lightweight \MixedCubes{} variant matches QuadCubes baseline quality at 5.1\,GB, approximately 11\% of the original memory requirement, making NeCT accessible for all CUDA-compatible consumer GPUs for the first time.

\textbf{Continuous-scan forward model.} The NeCT Beer--Lambert forward model was extended to account for continuous rotation during each exposure. A $K$-step midpoint approximation is derived that converges to the true angular integral with error scaling as $\mathcal{O}((\Delta\phi / K)^2)$. Two training step implementations are provided: a time-efficient mode with $\mathcal{O}(K)$ activation memory and a memory-efficient gradient checkpointing mode with $\mathcal{O}(1)$ memory overhead.

\textbf{Static validation of the forward model.} Experiments on a static Bentheimer sandstone sample confirmed that the $K$-step correction recovers reconstruction quality lost to angular blur for coarse acquisitions. The combined score for the 100-projection configuration rises from 0.242 at $K = 1$ to 0.400 at $K = 6$, with gains at every accumulation step following a diminishing pattern. For the 360-projection configuration the improvement is small throughout (0.593 to 0.708), indicating that at $1\degree$ angular step the forward model mismatch is not the dominant source of error. The 100-projection configuration does not reach 360-projection quality at any $K$, reflecting the independent contribution of projection count to reconstruction quality.

\textbf{Dynamic experiment and sample geometry.} The hourglass dynamic experiment showed no correlation between accumulation steps, angular step size, frame rate, and reconstruction quality. This result is attributed to the angular isotropy of the hourglass: its cylindrically symmetric cross-section means projections carry equivalent structural information from any angle, so angular blur does not degrade the information content of each projection and the correction has nothing to recover. This finding clarifies the scope of the continuous-scan correction. It is most beneficial for samples with fine directional structure, not for angularly symmetric ones.

\section{Limitations and Future Work}
\label{sec:conclusion_future}

The dynamic continuous-scan experiments already combined the two parts of this thesis, using \MixedCubes{} for the time-efficient training mode and QuadCubes for the memory-efficient mode. The combination was essential at higher accumulation steps, where the $\mathcal{O}(K)$ memory scaling of time-efficient training exceeds GPU capacity for the baseline QuadCubes configuration. Both modes produced the same null result for the hourglass dataset. Applying \MixedCubes{} with the continuous-scan forward model to a directionally structured dynamic sample remains an open experiment, and is the most direct test of whether the two contributions compound in reconstruction quality.

The dynamic continuous-scan experiment would benefit from a test dataset with directional structure. A rock sample undergoing anisotropic fracture propagation, a layered material experiencing differential dissolution, or a fibrous medium deforming under load would provide angular contrast that the hourglass lacked, and would allow the correction benefit demonstrated in the static experiment to be observed in a dynamic context as well.

The model currently assumes constant angular velocity within each projection's exposure interval. Extending the forward model to accept per-projection angular intervals rather than a global $\omega$ is a minor implementation change and would broaden applicability to instruments that vary rotation speed or use alternating-direction acquisition schemes.

Finally, reconstruction time remains on the order of hours on a single GPU. Possible directions for further reduction include exploiting multiple GPUs through data-parallel training, reducing the number of training iterations needed through better initialisation strategies, and investigating whether the dense temporal grids in \MixedCubes{} can be further compressed without quality loss by reducing the grid resolution at the coarsest levels.
