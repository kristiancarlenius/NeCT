...
This thesis builds directly on NeCT with two aims. The first is reducing the training time and memory cost of each reconstruction, and the second is extending the method to a new class of scan geometry. It investigates three questions the original paper left open. First, whether the encoder architecture is the optimal structural choice, and whether alternative designs can improve reconstruction precision or reduce model size. Second, whether the encoder configuration can be reduced in size without a proportional loss of reconstruction quality, identifying the smallest and fastest configuration that remains competitive. Third, whether the NeCT forward model can be extended to handle continuously acquired data in which the sample rotates without stopping between exposures, enabling information-denser acquisitions.
...
Figure~\ref{fig:combined_score} shows the combined score as a function of $K$ for both configurations. The results confirm the expected relationship between angular step size, correction depth, and reconstruction quality.

For the 100-projection configuration ($\Delta\phi = 3.60\degree$), the $K = 1$ score of 0.165 indicates severely degraded quality. The single-angle forward model produces substantial blur at this step size and the reconstruction captures little fine pore structure. Increasing $K$ produces consistent improvement at every step, reaching 0.306 at $K = 2$, 0.378 at $K = 3$, 0.441 at $K = 4$, and 0.500 at $K = 6$ (effective sub-step $0.60\degree$). The total gain of 0.335 points is substantially larger than for the 360-projection case. Notably, the score at $K = 6$ nearly matches the 360-projection score at $K = 1$ ($S = 0.492$, effective sub-step $1.00\degree$), which is consistent with the Shannon--Nyquist sampling theorem applied to the angular domain. What governs reconstruction quality is the density of angular samples within each exposure interval, expressed as the effective sub-step $\Delta\phi / K$, rather than the number of raw projections.

For the 360-projection configuration ($\Delta\phi = 1.00\degree$), the $K = 1$ score of 0.492 reflects usable quality even without correction, as the smaller angular step produces less blur. Increasing $K$ still improves the score steadily, reaching 0.543 at $K = 2$, 0.583 at $K = 3$, 0.613 at $K = 4$, and 0.647 at $K = 6$, a total gain of 0.155 points. Both curves increase monotonically with no decline at higher $K$, confirming that the approximation does not introduce new artefacts at large sub-step counts.

\todo{Insert reconstruction slices at $K=1$ and $K=6$ versus ground truth for both projection counts, and the matched-pair comparison table.}
...
The static results confirm that the $K$-step correction provides a genuine benefit when angular blur is the limiting factor. For the 100-projection configuration ($\Delta\phi = 3.60\degree$) the improvement from $K = 1$ to $K = 6$ is substantial, with the combined score rising from 0.165 to 0.500, confirming that angular blur at this step size is severe and that sub-step averaging recovers a large fraction of the lost information. For the 360-projection configuration ($\Delta\phi = 1.00\degree$), however, the benefit is marginal. The score at $K = 1$ is already 0.492 and rises to 0.647 at $K = 6$, but the improvement per additional sub-step diminishes rapidly after $K = 2$, suggesting that at $1\degree$ the angular drag is not the primary source of error in the reconstruction. The correction is approaching the limit of what sub-step averaging can recover for this projection count.

Importantly, the 100-projection configuration at $K = 6$ (combined score 0.500, effective sub-step $0.60\degree$) does not match the 360-projection configuration at $K = 1$ (0.492, $1.00\degree$). Although the scores are numerically close, the 100-projection reconstruction never reaches comparable spatial quality to the 360-projection reconstruction at any value of $K$ tested. This indicates that the Nyquist-motivated hypothesis that sub-step precision determines quality independently of projection count does not fully hold in practice. The total number of projections also matters, because a denser native angular sampling provides more independent views of the sample regardless of how finely each projection's swept interval is divided. The $K$-step correction addresses the angular smear within each projection but cannot recover the additional spatial information that additional projections would provide.

The approximation error of the midpoint rule scales as $\mathcal{O}((\Delta\phi/K)^2)$, meaning it shrinks by a factor of four each time $K$ is doubled. The monotonic improvement with $K$ for the 100-projection case and the absence of any quality decline at large $K$ confirm that the correction does not introduce new artefacts. The benefit is real, but bounded.

\subsection{Dynamic Experiment and Sample Geometry}
\label{subsec:cont_disc_dynamic}

The hourglass dynamic experiment produced a negative result: no meaningful correlation was found between accumulation steps $K$, angular step size $\Delta\phi$, frame rate, and reconstruction quality as measured by the volume MAE metric. This outcome is informative rather than simply a failure, and the most probable explanation lies in the geometric properties of the hourglass sample.
...
The hourglass results also highlight a fundamental limitation of the volume MAE metric for evaluating the angular blur correction: the metric is insensitive to the correction when the sample is angularly isotropic. A metric that directly measures projection residuals, or that evaluates reconstruction quality on a sample with known directional structure, would be more appropriate for validating the forward model in a dynamic setting. The static sandstone experiment, which uses PSNR-independent metrics against a high-quality reference scan, is a better-controlled test of the forward model itself.
...
\textbf{Static validation of the forward model.} Experiments on a static Bentheimer sandstone sample confirmed that the $K$-step correction recovers reconstruction quality lost to angular blur for coarse acquisitions. The combined score for the 100-projection configuration improves from 0.165 at $K = 1$ to 0.500 at $K = 6$. For the 360-projection configuration the benefit is present but marginal, indicating that at $1\degree$ angular step the blur is not the dominant source of error. The total projection count matters independently of sub-step precision, so the correction is most useful when the acquisition is genuinely coarse.
...
The continuous-scan forward model was tested in isolation from the encoder improvements, with all continuous-scan experiments using the original NeCT baseline encoder. Combining the two contributions is a natural next step. Applying the \MixedCubes{} architecture and reduced capacity settings to the continuous-scan training pipeline would allow the faster per-epoch training of \MixedCubes{} to compound with the higher effective projection density of continuous scanning, potentially enabling high-quality 4D reconstructions at substantially lower total training time than either improvement achieves independently.
