...
Expectidly each of the graphs show the same overall trend with higher precision correlating higher hashmap sizes although there are some clear exceptions like $L=22$ losing precision from $\log_2 T = 22$ to $\log_2 T = 23$, at $L=19$ reaching its highest precision at $\log_2 T = 20$ and several experiencing lower precision at $\log_2 T = 22$ than $\log_2 T = 21$.
...
Figure~\ref{fig:quality_vs_time} shows PSNR for all configurations evaluated in \cref{sec:enc_size_params} at an equalized wall-clock budget of approximately 14 hours, with curves grouped by number of levels and the x-axis showing total encoder parameter count.
...
At 14 hours of training we acheive the highest precision with $L=18, \log_2 T=21$ which has compleated 350 epochs by the time $L=23, \log_2 T=23$ has only finished 150 epochs. Many of the models have outcompeated the $L=23, \log_2 T=23$ base model, markably higher precision found with $(L=19, 22, 23 \log_2 T=20)$, $(L=17, 18, 20, 22, 23  \log_2 T=21)$, $(L=17, 18, 21, 22 \log_2 T=22)$, $(L=17, 18, 20, 22 \log_2 T=23)$. There seem to be alot of variance in the results so nothing conclusive can be said before more testing is done but it point in the direction that time optimal reconstructions could be reached by downsized encoders. There also seem to be a somewhat clear break point where the model becomes to small, failing to capture the complexity such that they are unable to able fully converge. 
...
\todo{Describe the results. At $K = 1$ for the 100-projection scan, angular smearing should be visible. As $K$ increases, quality should improve and plateau. State whether the matched-pair equivalences hold, e.g.\ whether 360 projections at $K = 4$ reaches the same quality as 1400 projections at $K = 1$.}
...
Each dataset was reconstructed with accumulation steps $K \in \{1, 2, 3, 4, 6\}$, giving five reconstructions per dataset and thirty reconstructions in total. Table~\ref{tab:dynamic_cont_configs} shows the effective angular sub-step $\Delta\phi / K$ for each combination.
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \end{adjustwidth}
    \caption{Dynamic reconstruction of the hourglass from dataset 1 (2750\degree rotation, 4 fps) at 150 epochs, shown at three time points. The sand level in the upper chamber visibly decreases over time as material flows to the lower chamber.}
...
\todo{For each of the six datasets, present PSNR/SSIM as a function of $K$. Describe how the improvement from increasing $K$ depends on $\Delta\phi$. For the datasets with $\Delta\phi = 13.75\degree$ the jump from $K = 1$ to $K = 2$ should be large, while for $\Delta\phi = 3.44\degree$ the improvement may be modest.}
 
\todo{State whether the 2 fps and 4 fps variants at the same $\Delta\phi$ give similar quality at the same $K$, to isolate the effect of angular blurring from temporal sampling density.}
 
\todo{Show reconstruction slices at matched $K$ values for different $\Delta\phi$ to illustrate the visual effect of the accumulation correction. For example, compare dataset 1 at $K = 1$ (sub-step $3.44\degree$) with dataset 2 at $K = 2$ (sub-step $3.44\degree$) and dataset 3 at $K = 4$ (sub-step $3.44\degree$) to test whether the same effective sub-step gives comparable quality regardless of how it was achieved.}
